...
- **Finding (#121, audit #3, Medium / data-integrity):** the four `daily_step_record` per-field writers in `StepRepositoryImpl` (`updateDailySteps` / `updateHealthConnectSteps` / `updateActivityMinutes` / `updateEscrow`) were each `getByDateOnce(date)` → `upsert(existing.copy(field=…))`. `@Upsert` rewrites EVERY column from the read snapshot, so two independent components writing the same date row concurrently — sensor `StepCounterService` (`Dispatchers.Default`), 15-min `StepSyncWorker`, and `StepCrossValidator` (HC), all different threads, no shared lock — clobber each other: the second writer persists the OTHER columns at the stale value it read, reverting the first writer's column. Verifier-confirmed impact is daily-column accounting drift (50k ceiling reload in `DailyStepManager.ensureInitialized`, `sumCreditedSteps` weekly-challenge undercount, stats/history), NOT wallet loss (the wallet is a separate atomic `player_profile` table).
- **Fix (mirrors the existing `incrementBattleSteps` template):** 4 column-targeted DAO queries — `setSensorAndCreditedSteps` / `setHealthConnectSteps` / `setActivityMinutes` / `setEscrow` — each `INSERT (all 10 cols) … ON CONFLICT(date) DO UPDATE SET <only-its-columns>` (INSERT half supplies every NOT NULL col because SQLite checks NOT NULL before ON CONFLICT; UPDATE half touches only this writer's columns; SET-not-increment to match the cumulative-total semantics). `StepRepositoryImpl` delegates the 4 methods to them. Each writer now mutates a disjoint column set → concurrent writers structurally cannot clobber → **no Mutex, no schema change** (entity/table unchanged → no migration, no `app/schemas` drift). `FakeDailyStepDao` got the 4 methods with copy-only-their-fields semantics.
- **TDD:** RED — added 9 `DailyStepDaoTest` cases (real Robolectric Room), confirmed `Unresolved reference` for the 4 new methods. GREEN — added the DAO queries + repo delegation + fake impls. The centerpiece test is the lost-update repro: 4 disjoint-column writers from a stale snapshot all survive (pre-fix the whole-row copy would revert each other). Also: per-method create-on-empty-table, HC-preserves-sensor, battle/boss-counter preservation, SET-not-increment. Then 3 existing `StepRepositoryImplTest` mock tests (which `verify(dao).upsert(captor)` on the OLD read-copy-upsert) failed as expected — rewrote them to verify the new delegation contract (`verify(dao).setX(...)` + `verify(dao, never()).upsert(any())`), +1 net (added an updateHealthConnectSteps + updateActivityMinutes case). 
- **Verification:** `./run-gradle.sh testDebugUnitTest` → **908/0/0/0** (899 + 9); full `lintDebug assembleDebug` → BUILD SUCCESSFUL; `git status app/schemas` clean (no schema drift, as expected — entity untouched). Production diff: 2 files (DailyStepDao +4 queries, StepRepositoryImpl delegation). 
- **Doc sync:** CLAUDE.md headline 899→908; CHANGELOG new Fix section; source-files (DailyStepDao / StepRepositoryImpl / FakeDailyStepDao + DailyStepDaoTest 5→14 + StepRepositoryImplTest 8→9); STATE objective rotation (+ marked #125/#126 MERGED) + known-issues (#121 fixed, #124/#127 remain) + new fragile-zone note; this RUN_LOG entry. No ADR (a data-layer hardening to an existing pattern, not a new architecture decision; ADR-0020's atomic-economy reasoning already covers the principle).
- **Next:** commit + push + PR + merge (both required CI gates), prune branch. Remaining audit Lows: #124 (billing sig verify — local-only, bypassable mitigation, needs the Play public key), #127 (dup missions — needs a unique index + schema v11→v12 bump, defer to a schema-touching wave), #128 tracker. Launch remains the external critical path.

## 2026-06-10 — Battle-perf audit Lows #125 + #126 fixed (TDD, branch fix/125-126-battle-perf)

- **Goal:** "what's next?" under ultracode. Launch is externally gated (closed-track soak + ≥12 testers + production-access application — all the user's to do; the 14-day window elapsed 2026-06-09). Chose the highest-value in-repo work during the soak: the battle-performance audit pair, both self-contained with no schema change (lowest risk mid-soak) and directly improving the 2×/4× tester experience.
- **#126 (spiral of death):** `GameLoopThread` accumulated `elapsed × speedMultiplier` and drained it one `TICK_NS` at a time with no ceiling. A long frame (GC pause / starved main thread / heavy `engine.update`) — amplified up to 4× — could demand dozens-to-hundreds of `update()` calls before the next render, each catch-up tick begetting more (visible freeze). **Fix:** pure `SimulationMath.clampAccumulator(accumulatorNs, tickNs)` + `MAX_CATCHUP_TICKS = 8`, wired into `GameLoopThread` after the speed-scaled accumulate. Clamp drops the excess whole-tick backlog but preserves the sub-tick remainder. **Ceiling rationale (re-tuned from an initial 5):** must engage ONLY on a genuine multi-frame stall, never on a slow-but-steady device — a 30 fps render at 4× legitimately needs ~7.9 ticks/frame (33 ms × 4) and must pass unclamped; 8 clears that and still bounds the worst case. TDD: 7 RED `SimulationMathTest` (unresolved ref) → GREEN.
- **#125 (getAliveEnemies allocation):** the per-frame `entities.filterIsInstance<EnemyEntity>().filter { it.isAlive }` allocates **two** intermediate lists and is called multiple times/tick (UW auto-trigger gate + each active BLACK_HOLE/POISON_SWAMP ongoing effect + once per orb via its `getEnemies` callback). **Fix:** single-pass `ArrayList` build (one allocation), behaviour-identical. **Key finding — rejected the audit's "compute once and reuse" recommendation:** `EnemyEntity.takeDamage` is NOT idempotent on a dead enemy (re-fires `onDeath` → `handleEnemyDeath` re-credits cash/steps + re-spawns SCATTER children), and BLACK_HOLE+POISON_SWAMP both kill enemies in the same `updateUWs` pass — a shared/stale alive-list would let POISON re-hit BLACK_HOLE's corpses and **double-credit the kill**. The live per-call `isAlive` re-filter is load-bearing. Pinned with 2 `R125` GameEngineTest guards (characterization: GREEN on current code, stay GREEN after refactor); the double-credit guard counts per-enemy `onDeath` invocations across two overlapping ongoing UWs. (First draft of that guard used the engine kill counter and read `0` because hand-built test enemies wire `onDeath = {}` — corrected to count `onDeath` directly, which captures the mechanism precisely.)
- **Verification:** `./run-gradle.sh testDebugUnitTest` → **899/0/0/0** (890 + 7 + 2); confirmed both `R125` tests actually ran (parsed the report). `./run-gradle.sh lintDebug assembleDebug` → BUILD SUCCESSFUL, lint clean (incl. the `HardcodedText`-as-error guard). Production diff: 3 files (+SimulationMath clamp, +1-line GameLoopThread wiring, getAliveEnemies single-pass rewrite), 234 insertions total incl. tests.
- **Doc sync:** CLAUDE.md headline count 890→899; CHANGELOG new Perf section; source-files (SimulationMath 7→8 functions + clampAccumulator, SimulationMathTest 27→34, GameLoopThread clamp note, GameEngineTest R125 guards); STATE objective rotation + known-issues (#125/#126 → fixed) + 2 new fragile-zone entries (no-cache invariant + frame-clamp ceiling); this RUN_LOG entry. No ADR (two bounded perf fixes, no architecture decision; the no-cache rationale lives in code comments + the fragile-zone note).
- **Next:** commit + push + PR + merge (both required CI gates: `build-and-test` + `connected`), then prune the branch. Remaining audit Lows after this: #121, #124, #127 (schema bump — defer to a wave that already bumps the schema), #128 tracker. Then the launch is still the critical path (external).

## 2026-06-10 — Dependabot dependency wave merged to main (6 bumps, 1 PR)

- **Goal:** "we have some dependabot stuff open in the repo, please sort it." (Pivoted off a started #121 audit-fix investigation at the user's request.)
- **Triage:** 6 open Dependabot PRs (#134–#139), **0 open Dependabot security alerts**. Each PR already green on both required CI lanes (`build-and-test` + `connected`). User chose: merge **all 6**, via a **verified integration branch**.
- **Why combined, not 6 merges:** `main` branch protection is `strict: true` (up-to-date required) + both checks required. The 4 version-catalog PRs all edit `gradle/libs.versions.toml`, so merging one staled the other 5 → either ~40 min of rebase+re-run cascades or an admin-bypass that never CI-tests the combined result. A single integration branch lands all 6 atomically **and** verifies the combined upgrade (the thing per-PR CI structurally cannot check).
- **What changed:** cut `deps/dependabot-wave-2026-06-10` off `main`; `git fetch` the 6 Dependabot head branches; `git cherry-pick` all 6 commits in order (the 4 toml bumps auto-merged cleanly on disjoint lines; the wrapper PR carried the binary `gradle-wrapper.jar` + `gradlew`/`gradlew.bat`/properties). Bumps: gradle-wrapper 9.3.1→**9.5.1**, junit5/junit-jupiter 5.11.4→**6.1.0** (major, test-only), navigation-compose 2.9.7→2.9.8, guava 33.4.0→33.6.0-android (CVE floor constraint), androidx.test.ext:junit 1.2.1→1.3.0, gradle/actions 6.1.0→6.1.1 (SHA-pinned, 4 workflow files).
- **Verification:** local combined gate on the new Gradle 9.5.1 (first run downloaded the distribution) — `./run-gradle.sh testDebugUnitTest` BUILD SUCCESSFUL with **890 tests / 0 failures / 0 errors / 0 skipped** (parsed the JUnit result XML to prove the major JUnit-6 bump silently skipped nothing); `./run-gradle.sh assembleDebug lintDebug` BUILD SUCCESSFUL (R8/packaging exercises navigation-compose + guava at runtime; lint clean). Pushed; opened **PR #142**; both required CI gates green (`build-and-test` + `connected`, 8m7s each); `mergeable: MERGEABLE` / `mergeStateStatus: CLEAN`.
- **Merge:** `gh pr merge 142 --squash --delete-branch` → merge commit `162f8be`. All 6 Dependabot PRs (#134–#139) auto-CLOSED (their changes landed); local `main` fast-forwarded; local + remote integration branch pruned. Confirmed: **0 open PRs, 0 open Dependabot alerts.**
- **Doc sync:** build-config-only PR (no production code/test/schema) → headline count unchanged at 890. Fixed hardcoded version drift: `CLAUDE.md` (Gradle 9.3.1→9.5.1; "JUnit 5"→"JUnit Jupiter" in Testing), `docs/steering/tech.md` (Gradle 9.5.1 + navigation 2.9.8 + JUnit row → "JUnit (Jupiter) 6.1.0, catalog key still junit5"). CHANGELOG new Build section. STATE objective rotation + this RUN_LOG entry. No ADR (a routine dependency bump is not an architecture decision).
- **Next:** resume the closed-track soak (the launch gates remain external), or pick up a remaining audit Low — #121 (`daily_step_record` lost-update, was mid-investigation), #124/#125/#126, or #127 (needs a schema bump).

## 2026-06-10 — Audit fix wave MERGED to main (5 PRs)

- **Goal:** merge the 5 audit-fix PRs (#118/#119/#122/#120/#123) to `main`.
- **What happened:** merged bottom-up. All 5 fixes are now on `main` (merge commit `1e3a9b4`); the 5 issues auto-CLOSED; a post-merge `clean testDebugUnitTest` on `main` is green at **890/0**.
  - #129 (#118) merged → `fbe241d`. #140 (#119) merged → `6b310ec`. #141 (#122) merged → `7c8a689`. #132 (#120) merged → `c52919e`. #133 (#123, + the session-checkpoint docs commit) merged → `1e3a9b4`.
- **Stacked-merge wrinkle (lesson):** merging #129 with `--delete-branch` **auto-CLOSED** the next stacked PR — GitHub closes a PR the instant its base branch is deleted (it does NOT auto-retarget a stacked child to the deleted base's target). This closed #130 (base `fix/118`), then later #131 (base `fix/119`) the same way before I corrected course. No work was lost — the head branches + commits stayed intact. **Recovery:** reopened the orphaned work as fresh PRs against `main` — **#140 ⇒ #119** (branch `fix/119-golden-upgrade-loss`), **#141 ⇒ #122** (branch `fix/122-economy-atomicity`), each cleanly diffing against the updated `main` (the parent's commits became shared history). #130/#131 left CLOSED with "superseded by …" comments. **Safe procedure adopted for the rest:** retarget the child PR to `main` + `update-branch` BEFORE deleting the parent — never `--delete-branch` while a dependent PR still points at that branch. #132/#133 went through that path (retarget → update-branch → watch both checks → merge without delete), then I deleted the leftover merged branches at the very end.
- **Verification:** every merge passed both required gates against a `main` base — `build-and-test` (~4-5 min: lint + unit + debug build + Room schema-drift guard) AND `connected` (~6-7 min: API-34 emulator instrumented suite). Branch protection on `main` is strict (both checks + up-to-date), so each retargeted PR was `update-branch`'d and re-run before merge. Post-merge: local `main` fast-forwarded to `1e3a9b4`; `clean testDebugUnitTest` = **890 tests, 0 failures/errors**; `gh issue view` confirms #118/#119/#120/#122/#123 all CLOSED; no audit-fix PRs remain open; all 5 feature branches deleted.
- **Doc sync:** STATE (objective → MERGED + known-issues "awaiting merge" → "merged + CLOSED"), this RUN_LOG entry. CLAUDE.md (890), CHANGELOG (5 fix sections), source-files were already correct from the per-PR commits — the merge changed no code, so nothing else was invalidated. No new ADR (ADR-0020 already covers #122; a merge is not a decision).
- **Next:** remaining audit items are all Low or deferred — #121 (`daily_step_record` per-field upsert, partially mitigated by #120), #124/#125/#126 (Low), #127 (needs a unique index + schema bump), #128 (30-Low tracker). None block the closed-track soak. Optional belt-and-suspenders: on-device repro of the #118/#119 battle fixes (the JVM/Robolectric suite covers them, but device confirmation of the ORBS-at-4x and GOLDEN-expiry paths would close the loop).

## 2026-06-10 — Audit fix wave: 5 stacked PRs for the Medium+ findings

- **Goal:** fix the highest-severity findings from the same-day multi-agent audit — the 1 High (#118) + the 4 Major (#119/#120/#122/#123) — via TDD, one PR per issue.
- **Approach:** Phase-1 root-cause re-verification of all 5 findings against HEAD was run as a parallel investigation workflow (5 investigators + per-plan adversarial reviewers) before any code change — this caught two material refinements the audit prose had under-specified (see below). Each fix then followed strict RED→GREEN: write the failing test, watch it fail for the right reason, implement the minimal fix, watch it pass, run the full suite. Delivered as a **linear stacked PR chain** (each branch off the previous) since all five touch `CHANGELOG.md` + the CLAUDE.md headline test-count line; GitHub retargets each PR's base to `main` as the chain merges.
- **What changed (5 PRs, all branched, full suite green at each step, 867 → 890 JVM):**
  - **#118 (High) → PR #129** `fix/118-battle-entities-race`. `GameEngine.entities` was a plain unsynchronized `mutableListOf` structurally mutated + iterated from both the GameLoopThread (`update`/`render`) and the main thread (in-round ORBS purchase via `applyStats`; playAgain via `init`). Added a private `entitiesLock` monitor guarding all four regions (`render` snapshots under the lock then draws outside); marked `stats` `@Volatile`. New `GameEngineConcurrencyTest` (2 stress tests) reproduced the **CME** (ORBS) and **IndexOutOfBounds** (re-init) in milliseconds pre-fix. 867 → 869.
  - **#119 (Major) → PR #130** `fix/119-golden-upgrade-loss`. GOLDEN_ZIGGURAT restored a stale `preGoldenStats` snapshot on expiry, discarding any in-round upgrade bought during its (auto-recurring) window. Modelled GOLDEN as a re-derived damage *layer* (`goldenDamageMult` + GOLDEN-aware `updateZigguratStats` that re-captures the base). 2 new GameEngineTest cases; the 2 existing fortune-stacking guards untouched. 869 → 871.
  - **#122 (Major) → PR #131** `fix/122-economy-atomicity` (**ADR-0020**). Covers report #4/#5/#9/#10. `PlayerRepository.spendGems`/`spendPowerStones`→`Boolean` + new `spendStepsIfSufficient`; every use case (`StartResearch`/`UnlockUltimateWeapon`/`UpgradeUltimateWeapon`/`UnlockLabSlot`/`OpenCardPack`/`RushResearch` + `StoreViewModel.purchaseCosmetic`) now gates the grant on the guarded deduct. `spendSteps` keeps its clamp (anti-cheat escrow). Guarded `markClaimed … AND claimed=0` (Int) + mark-first for supply drops; extracted a `ClaimMission` use case for the mission claim. +17 JVM incl. a real-SQLite `GuardedClaimDaoTest`. 871 → 888.
  - **#120 (Major) → PR #132** `fix/120-dailystepmanager-race` (report #6/#12). `DailyStepManager` `@Singleton` credit path now runs its read-check-write under a `kotlinx` `Mutex` (`ensureInitializedLocked` is the non-reentrant init split); `stepsPerMinute`→`ConcurrentHashMap`; counters `@Volatile`. Deterministic `DailyStepManagerConcurrencyTest` (test-only `onBeforeCreditCommit` seam). 888 → 889.
  - **#123 (Major) → PR #133** `fix/123-reboot-step-loss` (report #7). Reboot detection (`currentCounter < dayStart`) + re-baseline in `StepSyncWorker`, **plus** a `sensorStepsAtDayStart` offset so the gap is measured relative to the (re-anchored) baseline — without it, Room's same-day `sensorSteps` cumulative survives the reboot and the re-baseline alone still drops the steps. Extracted a pure `computeCatchUp(...)` companion so the test drives the **real** production arithmetic. 889 → 890.
- **Investigation-workflow refinements (caught before coding):** (#123) the load-bearing `sensorStepsAtDayStart` offset — re-baselining `dayStart` alone is insufficient because Room `sensorSteps` survives a reboot; (#120) the concurrency test must use a deterministic injected suspend-seam, not a probabilistic race, and lead with the ceiling-overshoot assertion. Both were applied.
- **Verification:** each fix's regression test confirmed RED against the unfixed code (for #118/#122/#120/#123 by reverting the production change and re-running — e.g. #120 overshot to 50,100, #123 credited 0 instead of 180) then GREEN after. Full `testDebugUnitTest` BUILD SUCCESSFUL after every PR. No Room schema/migration change in any of the five (the schema-drift CI gate stays green); the only new persisted state is one SharedPreferences key (#123). Baseline suite confirmed green (867) before starting.
- **Doc sync (per PR):** CHANGELOG section + CLAUDE.md headline count + `docs/steering/source-files.md` entries on every PR; ADR-0020 for #122; CLAUDE.md Battle-Renderer thread-safety note + Conventions atomic-spend rule updated. STATE objective rotation + known-issues audit update + fragile-zones additions + this RUN_LOG entry at session end.
- **Next:** review/merge the stack bottom-up (#129 → #130 → #131 → #132 → #133); each retargets to `main` automatically as the one below merges. Remaining audit items: #121 (Low, `daily_step_record` per-field upsert — partially mitigated by #120), #124/#125/#126 (Low), #127 (needs a unique index + schema bump — deferred), #128 (30-Low tracker). None block closed-track soak.

## 2026-06-10 — Multi-agent code audit + CLAUDE.md rewrite

- **Goal:** (1) run a thorough reachability-aware audit of the whole codebase, write a report, and raise/triage GitHub issues for the findings; (2) review and improve the auto-loaded `CLAUDE.md`.
- **What changed:**
  - **Audit (analysis + tracking only — no production code touched).** Orchestrated an 8-specialist workflow (correctness, concurrency, data/Room, sensor/anti-cheat, security, async, performance, supply-chain) over the recon-modelled repo, with an adversarial verifier dispatched per finding to disprove it. 50 raw → **45 confirmed (1 High / 14 Medium / 30 Low), 5 rejected**. Re-verified the High at source by hand. Report written to `docs/external-reviews/2026-06-10-multi-agent-code-audit.md` (193 KB, historical artifact — full evidence/verification/fix/regression-test per finding + the 5 rejected false-positives for calibration).
  - **Issues filed (root-cause deduped):** #118 (entities cross-thread crash — High; merges report #1+#15), #119 (GOLDEN snapshot loses upgrades), #120 (DailyStepManager singleton race — merges #6+#12), #121 (daily_step_record lost-update), #122 (economy spend/claim atomicity — merges #4+#5+#9+#10), #123 (reboot step-loss), #124 (no purchase signature verification), #125 (getAliveEnemies per-frame alloc), #126 (game-loop spiral of death), #127 (duplicate daily missions), #128 (consolidated 30-Low tracker). Cross-referenced epics #22/#26/#21 with comments; linked #118↔#120↔#121. Also deleted the stale `docs/external-reviews/` folder the user had emptied (commit `12605b3`) before re-creating it for this report (`36be559`).
  - **CLAUDE.md rewrite.** ~48 KB/347 lines → ~18 KB/267 lines (−63%). Cut the 18 KB "Current coverage" changelog paragraph, the duplicated Current-Status checklist, inline version/migration history, and the full plan index/dep-graph/critical-path (→ pointers to master-plan/STATE/CHANGELOG). Fixed drift: regenerated the architecture map from the real tree (added the `domain/battle/` simulation layer + `data/ads|billing|anticheat|time` + `presentation/audio|help|store` + the 8-module `di/` list), removed all dead Step-Overdrive / Card-Dust / `ActivateOverdrive` references, dropped stale type counts, reconciled 36↔38 plan entries, pointed Version at `build.gradle.kts`. Added "Keeping this file honest" (anti-drift rule) + "Known fragile zones & active risk" (pointer to open issues) sections; resolved the START_HERE.md overlap; added the atomic-guarded-deduct convention + a battle thread-safety warning.
- **Verification:** audit findings cross-checked against source (e.g. confirmed `purchaseInRoundUpgrade` is main-thread non-suspend, `GameLoopThread.update()` runs outside `synchronized(surfaceHolder)`, ORBS is in-round-purchasable). CLAUDE.md post-edit checks: size −63%; all dead terms gone (greps clean except the two intentional "was removed" mentions); all 13 referenced doc paths exist; code fences balanced; `structure.md` confirmed already-accurate (left untouched). No `.kt`/`.kts`/`.toml`/schema file changed — **867 JVM + 9 instrumented unchanged**, no build run needed (docs + issues only).
- **Doc sync:** CHANGELOG (new Docs section), STATE (objective rotation + known-issues audit entry), this RUN_LOG entry, CLAUDE.md (the rewrite itself). No ADR — neither piece is a non-trivial architecture decision (the rewrite follows the existing "authority here, progress in the spine" principle; the audit is analysis).
- **Next:** fix #118 (the reachable battle crash) + #122 (economy atomicity) as the highest-value code follow-ups, or resume closed-track soak. The 30-Low tracker (#128) is patch-backlog.
...
- **Key design finding:** `GameEngine.onProjectileHitEnemy` **mutates enemy position** (`applyKnockback` when `knockbackForce > 0`) and sets `proj.isAlive = false`. So a naive "detect all pairs, then dispatch all callbacks" refactor would change behaviour when two projectiles target the same enemy in one frame (the original re-reads the enemy's post-knockback position for the second projectile). The fix had to **preserve the interleaved detect-and-fire loop**. Achieved by making the sweep *generic* over `EntityProtocol` and passing the callback in — `onHit` fires synchronously inside the loop reading live positions, exactly as before.
- **Outcome:** landed on branch `feat/V1X-09-phase3-collision` (from `main` HEAD `ea4530b`). The collision iteration + overlap geometry now live in `Simulation`; `CollisionSystem` is a thin presentation adapter. `testDebugUnitTest` + `assembleDebug` BUILD SUCCESSFUL, 851 → 857 (+6). No behaviour change.
- **Changes (production):**
  - **`EntityProtocol.kt`**: +`val x` / `val y` / `val width` (the positional reads the sweep needs). Doc updated to mention the collision sweep.
  - **`Entity.kt`**: `x` / `y` / `width` become `override var` (satisfy the protocol); `height` stays a plain `var` (not in the protocol — collision doesn't read it).
  - **`Simulation.kt`**: +`import kotlin.math.hypot`; +`detectProjectileEnemyHits<P, E : EntityProtocol>(projectiles, enemies, onHit)` (first overlapping enemy per projectile + `break`) and +`detectZigguratHits<P : EntityProtocol>(enemyProjectiles, zigX, zigY, zigWidth, onHit)`. Both keep the EXACT original overlap expressions (`(proj.width + enemy.width) / 2f` and `zigWidth / 2f + proj.width / 2f`) and fire `onHit` interleaved inside the loop.
  - **`CollisionSystem.kt`**: rewritten as a presentation adapter — takes a `Simulation` param, keeps `filterIsInstance` + the alive snapshot, delegates both sweeps. The `hypot` import + loops are gone from here.
  - **`GameEngine.kt`**: the `CollisionSystem.checkCollisions(...)` call site (inside `update()`) now passes `simulation` as the first arg. (`private val simulation` already existed from slices 1–3.)
- **Changes (test):**
  - **`SimulationTest.kt`**: `FakeEntity` gains `override var x/y/width` (defaults `0f`, declared before `isChronoSlowable` so the existing named-arg `tickEntities` call sites still resolve). +6 cases: detectProjectileEnemyHits overlap-fires-pair / first-hit-break / no-overlap-no-fire / interleaved-firing guard (the `onHit` moves the enemy mid-sweep, proving a later projectile misses — directly guards the knockback-equivalence reasoning above); detectZigguratHits hit / miss. 20 → 26.
- **Verification:** `./run-gradle.sh testDebugUnitTest` → BUILD SUCCESSFUL, 857 (851 + 6); `./run-gradle.sh assembleDebug` → BUILD SUCCESSFUL. test-results sum confirmed 857. Only 2 `EntityProtocol` implementers (`Entity` prod + `FakeEntity` test), both updated.
- **Doc-sync:** ADR-0012 (slice-4 per-slice bullet + remaining-slices line trimmed to UW lifecycle + `SimulationEvent`), `.kiro/steering/source-files.md` (Simulation / EntityProtocol / Entity / CollisionSystem / SimulationTest entries), AGENTS.md (count 851 → 857 ×2, V1X Phase 3 → slices 1–4, coverage slice-4 clause), CHANGELOG.md (new Phase 3 (cont.) section), README.md (status banner + test count), STATE.md (this rotation), RUN_LOG.md (this entry).
- **Merge:** committed on `feat/V1X-09-phase3-collision`, pushed, PR opened + merged to `main` (PR #96).
- **Remaining Phase 3:** UW lifecycle, then the `SimulationEvent` flow replacing the `@Volatile` polling callbacks. Each its own PR.

## 2026-06-01 — V1X-09 Phase 3 (slice 2): round-progress counters extracted to domain Simulation

- **Goal:** "merge please then continue" — after merging slice 1 (PR #93), continue Phase 3 with the next safe slice. The "real" remaining work (entity tick + collision) needs an `EntityProtocol` design and deserves its own focused PR; the natural sibling of the cash slice is the round-progress counters.
- **Outcome:** slice 1 (PR #93) merged to `main` via `ceb08dd` first. Then slice 2 landed on branch `feat/V1X-09-phase3-round-progress`: the round-progress counters (`totalEnemiesKilled` / `totalStepsEarned` / `elapsedSeconds`) + `hasWaveProgress()` now live in `Simulation`. `testDebugUnitTest` + `assembleDebug` BUILD SUCCESSFUL, 840 → 847 (+7). No behaviour change.
- **Changes (production):**
  - **`Simulation.kt`**: +3 `@Volatile private set` fields (`totalEnemiesKilled` / `totalStepsEarned` / `elapsedSeconds`) + `tickElapsed(dt)` / `recordEnemyKilled()` / `creditSteps(amount)` (ignores non-positive) / `hasWaveProgress()`; `reset()` extended to zero them. Class KDoc updated to "in-round state owner (growing per Phase 3 slice)".
  - **`GameEngine.kt`**: the 3 round-counter fields become get-only delegating props onto `simulation`; `hasWaveProgress()` delegates; `update()` clock → `simulation.tickElapsed(deltaTime)`; `handleEnemyDeath` → `simulation.recordEnemyKilled()` + `simulation.creditSteps(stepReward)`; `init()` drops the now-redundant counter-zeroing line (handled by `simulation.reset()`). **`roundOver` deliberately stays in the engine** — it's a control flag gating `update()` and written externally by `quitRound`/`onCleared`, so it belongs with the tick loop (which moves in a later slice).
- **Changes (test):**
  - **`SimulationTest.kt`**: +7 cases (tickElapsed accumulate, recordEnemyKilled increment, creditSteps accumulate + ignore-non-positive, hasWaveProgress false-on-fresh / true-after-tick / true-after-kill, reset zeroes counters). 9 → 16.
  - **`BattleViewModelTest.installEngineWithProgress`**: previously reflected on `getDeclaredField("elapsedTimeSeconds")` / `("totalEnemiesKilled")` (B.3 PR 2 mid-nav-persistence tests) — same backing-field breakage as the cash helper. Reworked to reach the engine's `simulation` field and call `tickElapsed` / `recordEnemyKilled` reflectively. (This time the fix went in pre-emptively, so the suite was green on the first `testDebugUnitTest` run.)
- **Verification:** `./run-gradle.sh testDebugUnitTest` → 847 green; `./run-gradle.sh assembleDebug` → BUILD SUCCESSFUL. `find … test-results … | awk` sum confirmed 847.
- **Doc-sync:** AGENTS.md (count 840 → 847 ×2, Phase 3 → slices 1–2, coverage slice-2 clause), `.kiro/steering/source-files.md` (Simulation.kt + SimulationTest.kt entries), `.kiro/steering/structure.md` (engine/ note), `CHANGELOG.md` (new Phase 3 (cont.) section), ADR-0012 (round-progress per-slice bullet), STATE.md (this rotation), RUN_LOG.md (this entry).
- **Merge:** committed on `feat/V1X-09-phase3-round-progress`, pushed, PR opened + merged to `main` this session (per the user's "merge please then continue").
- **Remaining Phase 3:** the substantial slice — entity tick + collision migration (needs a pure `EntityProtocol` so the domain `Simulation` can iterate entities without `Canvas`) — plus UW lifecycle and the `SimulationEvent` flow replacing the `@Volatile` callbacks. Each its own PR.

## 2026-06-01 — V1X-09 Phase 3 (slice 1): in-round cash economy extracted to domain Simulation

- **Goal:** "start phase 3" — begin V1X-09 Phase 3 (lift the `GameEngine.update()` simulation into a pure-domain `domain/battle/engine/Simulation`; ADR-0012). Following the per-slice PR cadence established by Phase 2 (#89–#92) rather than a big-bang rewrite, since the plan flags Phase 3 as a substantial refactor needing a dedicated review window.
- **Outcome:** first slice landed on branch `feat/V1X-09-phase3-simulation-cash` (from `main` HEAD `9abd3ae`). The in-round **cash economy** now lives in a new pure-domain `Simulation` class; `GameEngine` delegates its `cash` / `totalCashEarned` / `spendCash` public surface to it. `testDebugUnitTest` + `assembleDebug` BUILD SUCCESSFUL, 831 → 840 (+9). No behaviour change. Not yet committed (awaiting user PR decision, matching the Phase 2 cadence).
- **Why the cash economy first:** it's the most self-contained piece of engine simulation state — pure `Long` arithmetic with zero entity/Canvas coupling — so it establishes the `Simulation` class + delegation pattern cleanly (mirroring how `ProjectileState` seeded the Phase 2 entity-state package) with minimal blast radius.
- **Changes (production):**
  - **`domain/battle/engine/Simulation.kt`** (NEW, 70 lines, no Android imports): `@Volatile cash` / `totalCashEarned` (both `private set`) + `reset()` / `creditCash(amount)` (credits both balances, ignores non-positive) / `applyInterest(level)` (`+min(level × 0.5 %, 10 %)` onto `cash` only — interest excluded from lifetime, matching pre-extraction behaviour) / `spend(amount)` (insufficient-balance guard).
  - **`presentation/battle/engine/GameEngine.kt`** (7 edits): added `Simulation` import; dropped now-unused `kotlin.math.min` import; replaced the two `@Volatile var cash` / `totalCashEarned` fields with `private val simulation = Simulation()` + get-only delegating props; `init()` → `simulation.reset()`; `spendCash` → `simulation.spend`; `handleWaveComplete` → `creditCash` + `applyInterest`; `handleEnemyDeath` → `creditCash`. The per-kill + wave-complete reward *formulas* (which read tier config / enemy type / `fortuneMultiplier` / `cashResearchMultiplier` / `cashBonusPercent`) stay in `GameEngine`. `BASE_CASH_PER_WAVE` / `FLAT_BONUS_PER_WAVE_LEVEL` companion constants kept in `GameEngine` (referenced by `CashEconomyTest`).
- **Changes (test):**
  - **`domain/battle/engine/SimulationTest.kt`** (NEW, 9 pure-JVM tests, JUnit 5): creditCash accumulate / sum / ignore-non-positive; applyInterest add-to-cash-not-lifetime / cap-10 % / level-0-no-op; spend affordable / insufficient-leaves-balance; reset zeroes both.
  - **`BattleViewModelTest.installEngineForPurchase`**: the reflective helper previously did `engine.javaClass.getDeclaredField("cash")` to seed cash — which now throws `NoSuchFieldException` because `cash` is a delegating property with no backing field. Reworked to reach the engine's private `simulation` field and call its public `creditCash(Long)` reflectively. (This was the only reflection site touching the old `cash` field — confirmed via grep; the 3 RO-12 tests using it failed on the first test run and pass after the fix.)
- **Verification:**
  - First `./run-gradle.sh testDebugUnitTest` → compiled clean (840 tests ran) but 3 `BattleViewModelTest` RO-12 cases failed with `NoSuchFieldException at BattleViewModelTest.kt:891` (the helper above). Fixed the helper, re-ran.
  - `./run-gradle.sh testDebugUnitTest` → BUILD SUCCESSFUL, 840 tests (831 + 9 new), 0 failures.
  - `./run-gradle.sh assembleDebug` → BUILD SUCCESSFUL.
  - LSP `get_diagnostics` was unreliable (reported every `android.*` + entity import unresolved — workspace classpath not indexed); Gradle is the source of truth, as in prior sessions.
- **Doc-sync (per agent protocol PR Task-List Convention):** `AGENTS.md` (V1X test count 656 → 840 / Phase 3 STARTED note / coverage line 831 → 840 + Simulation slice clause), `.kiro/steering/source-files.md` (Simulation.kt + SimulationTest.kt entries), `.kiro/steering/structure.md` (engine/ dir note adds Simulation), `CHANGELOG.md` (new "V1X-09 Phase 3 (start)" section under [Unreleased]), `docs/agent/DECISIONS/ADR-0012-simulation-extraction.md` (Phase 3 flipped deferred → IN PROGRESS + per-slice cash-economy bullet + remaining-slices list), STATE.md (this rotation), RUN_LOG.md (this entry). README left untouched — test count there is informational, not a build/run instruction, and prior Phase 2 slices left it lagging.
- **Remaining Phase 3 slices:** entity tick + collision migration (needs a pure `EntityProtocol` so the domain `Simulation` can iterate entities without `Canvas`), UW lifecycle, and the `SimulationEvent` flow replacing the `@Volatile` callbacks. Each as its own per-slice PR.
- **Next session:** commit + push branch + open PR + merge. Then the next Phase 3 slice — or pause for the external closed-track soak per user direction.
...
## 2026-05-23 (late) — End-of-Wave-1 AAB v9 build + upload + on-device verification

- Goal: close out Plan R4 Wave 1 by bundling all four merged sub-plans (R4-01 / R4-02 / R4-02b / R4-04) into a release AAB, uploading to Play Console internal track, and verifying on-device.
- Context preflight: post-merge state on `main` is at commit `cbbb525` ("build: bump versionCode 8 -> 9 for end-of-Wave-1 AAB v9") with PR #12 (R4-02b) just merged via `755c90b`. `git status` clean. Test count 616 (per the previous run's `./run-gradle.sh testDebugUnitTest`).
- Build + sign (user-driven this session, agent observed):
  - `app/build.gradle.kts`: `versionCode = 9`, `versionName = "1.0.0"`. Single-line bump from 8 → 9 already committed.
  - `./run-gradle.sh clean bundleRelease` — BUILD SUCCESSFUL. R8 + lintVital + signing + native-debug-symbols bundling all clean. Output AAB at `app/build/outputs/bundle/release/app-release.aab` (~19 MB).
  - Signed with the upload keystore at `release/upload-keystore.jks` (gitignored).
  - `jarsigner -verify` confirmed signed (self-signed + no-timestamp warnings expected for upload keys; Play App Signing re-signs with Google's key after upload).
- Upload + verify (user, off-agent): AAB v9 uploaded to Play Console internal track 2026-05-23. On-device smoke test PASSED 2026-05-23.
- Wave 1 sub-plan inventory bundled in v9:
  - **R4-01** Remove Step Overdrive (PR #9, commit `e375d14`, 627 → 615 tests)
  - **R4-02** Multishot/Bounce 4-level scaling (PR #10, commit `b2f7cd5`, 615 tests unchanged)
...
- AAB v8 (built earlier 2026-05-23 ahead of the R4-02b amendment, versionCode 8) was **discarded without upload** — the R4-02b amendment landed before v8 left the developer's machine, and v9 supersedes.
- Doc-sync per agent protocol PR Task-List Convention:
  - `AGENTS.md`: R4 status entry rewritten to "Wave 1 SHIPPED + VERIFIED" with the v9 upload + smoke-test confirmation line; Tech Stack version line rewritten to versionCode 9 with the historical-sequence summary (v3 → v7 → v8 discarded → v9).
  - `README.md`: Status line rewritten for AAB v9 verified on internal track; Wave 2 next; 616 JVM tests (was 627).
  - `CHANGELOG.md`: new milestone section at top of `[Unreleased]` titled "Plan R4 Wave 1 — AAB v9 verified on internal track (2026-05-23)" listing all four bundled sub-plans with their PR numbers + commits + test deltas.
  - `STATE.md`: Current objective rewritten to "Wave 1 SHIPPED + VERIFIED. AAB v9 uploaded ... on-device smoke test PASSED ... Next: start R4-03 (Rapid Fire) on a new branch as the first Wave 2 sub-plan." Previous-objectives stack rotated to push R4-02b + AAB v9 build/upload entries onto the head. Top priorities + Next actions fully reframed for Wave 2 (R4-03 → R4-06 → R4-07 → end-of-Wave-2 AAB v10 → Wave 3 → Wave 4 → closed track → ≥14 days × ≥12 testers → production access). Last-run rotated.
  - No changes to `.kiro/steering/source-files.md` (no source files added/removed/changed in this session — pure release-engineering milestone).
  - No changes to `.kiro/steering/structure.md` (no architectural shift).
  - No changes to `.kiro/steering/tech.md` (versionCode bump documented in AGENTS.md Tech Stack line; library versions unchanged).
- Open questions: none. Wave 2 is fully specced in `docs/plans/plan-R4-feedback-bundle.md` §R4-03 / §R4-06 / §R4-07. R4-03 is the obvious starting point next session — pure code change, no schema, no ADR, ~2-3 days estimated.
- Follow-ups created: cut branch `feat/R4-03-rapid-fire` from `main` next session. Plan section §R4-03 has the full spec — 10 levels, baseCost 2,000 Steps × 1.18× scaling, periodic-pulse engine pattern mirroring `tickRecoveryPackages`, ~6 GameEngineTest entries, 2 DescribeUpgradeEffectTest entries.
- Memory updated: STATE ✅ / RUN_LOG ✅
- ADR: not warranted — release-engineering milestone only, no architectural decision.

## 2026-05-23 (later, post-AAB-v9) — R4-03 (Rapid Fire upgrade) implementation — first Wave 2 sub-plan

- Goal: ship the first sub-plan of Plan R4 Wave 2 — new `RAPID_FIRE` Workshop upgrade in the ATTACK category that fires a periodic attack-speed burst during a wave's SPAWNING phase. User feedback: *"Add 'Rapid Fire', upgradable duration and speed, auto triggers at time interval (max upgrade should be constant)."* Locked decision (2026-05-22T20:27 BST): periodic-pulse engine pattern mirroring RECOVERY_PACKAGES; baseCost 2,000 Steps × 1.18× scaling × 10 levels; L1 60s/5s/2.0× → L10 30s/30s/3.0× linear-interpolated, with duration matching interval at L10 producing a permanent buff.

- Context preflight: synced `main` (`cbbb525` "build: bump versionCode 8 -> 9 for end-of-Wave-1 AAB v9"); cut branch `feat/R4-03-rapid-fire`. Read `UpgradeType.kt` (existing 23 entries + R4-02b `isWorkshopVisible` flag), `GameEngine.kt` (existing `tickRecoveryPackages` shape + `recoveryTimer` lifecycle in `init` / `update` / phase-reset), `ZigguratEntity.kt` (existing `attackInterval` computed property reading live stats), `DescribeUpgradeEffect.kt` (existing `format` `when` branches and the cash-utility cap-clamping pattern), `InRoundUpgradeMenu.kt` (existing `hiddenInRound` set with STEP_MULTIPLIER + RECOVERY_PACKAGES), `WaveSpawner.kt` (`enum class WavePhase { SPAWNING, COOLDOWN }`; `var phase: WavePhase = WavePhase.SPAWNING; private set`).

- Source changes (5 files, 1 new):
  - **NEW `domain/model/RapidFireSchedule.kt`** — pure-Kotlin interpolation helper with 6 `const val` constants (`INTERVAL_L1` 60f, `INTERVAL_L10` 30f, `DURATION_L1` 5f, `DURATION_L10` 30f, `MULTIPLIER_L1` 2f, `MULTIPLIER_L10` 3f) and 4 functions (`interval(level)`, `duration(level)`, `multiplier(level)`, `isPermanent(level)`). Centralises math so `GameEngine.tickRapidFire` and `DescribeUpgradeEffect.formatRapidFire` always read identical numbers — same pattern lesson as the R3-02 callback-chain widening, which fixed a similar "two sites, two interpretations" drift.
  - **`domain/model/UpgradeType.kt`** — added `RAPID_FIRE(UpgradeCategory.ATTACK)` enum entry (after `DAMAGE_PER_METER`) and `RAPID_FIRE to UpgradeConfig(2_000, 1.18, 10, 1.0, "Periodic attack-speed burst (60s/5s/2.0× → permanent/3.0× at max)")` row in the configs map. ATTACK group is now 9 entries (was 8). Total enum is 24 entries (was 23). MULTISHOT/BOUNCE_SHOT remain `isWorkshopVisible = false` from R4-02b; RAPID_FIRE keeps the default `true` so it surfaces on the Workshop screen.
  - **`presentation/battle/entities/ZigguratEntity.kt`** — added `@Volatile var rapidFireMultiplier: Float = 1f` field; modified `attackInterval` from `(1.0 / stats.attackSpeed).toFloat()` to `(1.0 / (stats.attackSpeed * rapidFireMultiplier)).toFloat()`. The multiplier is set by `GameEngine.tickRapidFire` while a burst is active and reset to `1f` when the burst expires or the wave enters cooldown phase. Field is on the entity (not the engine) so the existing `attackInterval` computed-property pattern propagates the multiplier without a new indirection.
  - **`presentation/battle/engine/GameEngine.kt`** — added `import com.whitefang.stepsofbabylon.domain.model.RapidFireSchedule`; added two new private fields (`rapidFireTimer: Float = 0f`, `rapidFireActiveRemaining: Float = 0f`) right after the existing `recoveryTimer` field with KDoc explaining the invariant; reset both fields in `init()` next to the existing `recoveryTimer = 0f`; called `tickRapidFire(deltaTime)` from `update()` immediately after `tickRecoveryPackages(deltaTime)`; added new `tickRapidFire(deltaTime)` private method right after `tickRecoveryPackages`. The new method mirrors `tickRecoveryPackages` shape: SPAWNING-phase only, level-0 no-op, COOLDOWN phase resets all three fields (`rapidFireTimer`, `rapidFireActiveRemaining`, and `zig.rapidFireMultiplier`). State machine: (1) decrement active-burst countdown, reset multiplier to `1f` when zero; (2) increment timer, fire when crosses interval (reset timer, arm active countdown, set multiplier, emit `FloatingText("RAPID FIRE!", DEFAULT_COLOR)`). Order matters — active-decrement runs *before* the timer-fire check so at L10 (interval == duration) the multiplier transitions from one burst to the next within a single tick with no observable 1-frame gap of `1f`.
  - **`domain/usecase/DescribeUpgradeEffect.kt`** — added `import com.whitefang.stepsofbabylon.domain.model.RapidFireSchedule`; added `RAPID_FIRE` branch in the `format` `when` (after `RECOVERY_PACKAGES`); added new `formatRapidFire(level)` private helper. Readout shape: `"inactive"` at L0; `"{i}s/{d}s/{m}×"` between L1 and L9 (e.g. `"60s/5s/2.0×"` at L1, `"33s/27s/2.9×"` at L9); `"permanent/{m}×"` at L10 once `RapidFireSchedule.isPermanent` returns true. Format strings pinned to `Locale.ROOT` via the existing `fmt(...)` helper. Reads workshop level only because RAPID_FIRE is hidden from in-round (per `InRoundUpgradeMenu.hiddenInRound`); the `total = workshopLevels[type] ?: 0 + (inRoundLevels[type] ?: 0)` pattern is preserved for symmetry but the in-round addend is always 0 in practice.
  - **`presentation/battle/ui/InRoundUpgradeMenu.kt`** — added `UpgradeType.RAPID_FIRE` to the `hiddenInRound` set alongside `STEP_MULTIPLIER` and `RECOVERY_PACKAGES`; updated the `// Mirror the WorkshopViewModel hidden-set` comment block to explain the new entry; cleaned up a stale orphaned RO-08 comment fragment that ended up between the new comment block and the `val upgrades = ...` filter line during the in-place edit (post-edit clean-up commit).

- Test changes (4 files, 0 new):
  - **`presentation/battle/engine/GameEngineTest.kt`** — 6 new `R403 ...` test entries inserted after the GOLDEN expiry test, before the helpers section: (1) level-0 produces no burst; (2) L1 fires after 60 s and sets multiplier to 2.0×; (3) L1 burst expires after 5 s duration and resets multiplier to 1f; (4) L5 interpolation produces ~2.444× (`assertEquals(2f + 4f / 9f, ..., 0.001f)`); (5) L10 produces permanent buff across burst boundaries (two consecutive 30 s ticks both keep multiplier at 3.0×); (6) COOLDOWN phase resets the timer (partial 30 s timer in SPAWNING + cooldown tick + back to SPAWNING + 30 s tick should not fire the burst because the timer reset on COOLDOWN). 2 new reflective helpers added to the helpers section: `invokeTickRapidFire(eng, deltaTime)` mirrors `invokeTickRecovery` (private-method `getDeclaredMethod` + `apply { isAccessible = true }` + `invoke`); `setWavePhase(eng, phase)` flips the engine's `WaveSpawner.phase` (private set) without ticking through the full 26 + 9 s wave cycle.
  - **`domain/usecase/DescribeUpgradeEffectTest.kt`** — 4 new `R403 ...` readout tests inserted right before the smoke test: (1) L0 shows `"inactive"` with L1 next-preview `"60s/5s/2.0×"`; (2) L1 shows `"60s/5s/2.0×"` with L2 next `"57s/8s/2.1×"`; (3) L9 shows `"33s/27s/2.9×"` with L10 next `"permanent/3.0×"`; (4) L10 shows `"permanent/3.0×"` with `assertNull(r.next)` because the upgrade is at maxLevel. The existing smoke test (`every visible upgrade type produces a non-empty current readout at L0`) implicitly covers RAPID_FIRE since `formatRapidFire(0)` returns `"inactive"` (non-empty).
  - **`domain/model/UpgradeTypeTest.kt`** — entry-count assertion bumped 23 → 24 (test name renamed `23 entries exist` → `24 entries exist`); ATTACK count assertion bumped 8 → 9 (test name renamed `category counts are 8 attack 9 defense 6 utility` → `category counts are 9 attack 9 defense 6 utility`).
  - **`presentation/workshop/WorkshopViewModelTest.kt`** — sanity assertion in the existing R4-02b filter test bumped from `6` to `7` ATTACK upgrades (`assertEquals(7, attackTypes.size, ...)`); comment updated to list the 7 expected entries (DAMAGE / ATTACK_SPEED / CRITICAL_CHANCE / CRITICAL_FACTOR / RANGE / DAMAGE_PER_METER / RAPID_FIRE) and noting the count history (was 6 in R4-02b, 8 in R4-02).

- Verification:
  - `./run-gradle.sh testDebugUnitTest` — BUILD SUCCESSFUL on **first run** with **626 tests** (was 616, +10 net: 6 GameEngineTest + 4 DescribeUpgradeEffectTest; the 2 UpgradeTypeTest renames + 1 WorkshopViewModelTest assertion change are in-place edits, not new tests).
  - `./run-gradle.sh assembleDebug` — BUILD SUCCESSFUL.

- Acceptance criteria (`docs/plans/plan-R4-feedback-bundle.md` § R4-03):
  - Purchasing RAPID_FIRE L1 produces a visible 5-second yellow `"RAPID FIRE!"` burst every 60 s with ~2× projectile rate → engine math regression-guarded; on-device check pending Wave 2 AAB.
  - L10 produces continuous yellow tint and ~3× projectile rate → regression-guarded by the L10-permanent-buff GameEngineTest.
  - `formatRapidFire` readout matches the spec (`"inactive"` / `"{i}s/{d}s/{m}×"` / `"permanent/{m}×"`) → 4 readout tests guard the format.
  - Hidden from in-round Cash menu (matches the "passive periodic effect" feature design) → `InRoundUpgradeMenu.hiddenInRound` set verified by code inspection.

- Doc-sync per agent protocol PR Task-List Convention:
  - `AGENTS.md`: R4 status entry rewritten to "Wave 1 SHIPPED + VERIFIED; Wave 2 IN PROGRESS"; **R4-03 (Rapid Fire) landed** detail block; coverage line bumped 616 → 626.
  - `README.md`: Status line updated to "Wave 2 in progress — R4-03 landed on branch"; Build commands test count bumped 627 → 626 (the 627 number was a stale Wave 1 doc bookkeeping error from before the AAB v9 milestone CHANGELOG sweep — caught and corrected here).
  - `CHANGELOG.md`: new `### R4-03: Rapid Fire upgrade (2026-05-23)` section at top of `[Unreleased]` above the Wave 1 milestone block. Full spec table, files-added/changed inventory, test-count delta breakdown, acceptance checklist, ADR-not-warranted note.
  - `.kiro/steering/source-files.md`: `UpgradeType.kt` count bumped 23 → 24 (with R4-03 attribution); new `RapidFireSchedule.kt` entry added immediately below; entries for `GameEngine.kt`, `ZigguratEntity.kt`, `DescribeUpgradeEffect.kt`, `InRoundUpgradeMenu.kt`, `GameEngineTest.kt`, `DescribeUpgradeEffectTest.kt`, `UpgradeTypeTest.kt`, `WorkshopViewModelTest.kt` left as-is (their existing entries cover the file's responsibility shape; per-method R4-03 specifics live in the CHANGELOG entry).
  - `.kiro/steering/structure.md`: no changes (no new directories/modules/architectural elements; RapidFireSchedule.kt fits the existing `domain/model/` shape).
  - `STATE.md`: Current objective rotated onto R4-03 landed; `What works` line updated to 626 with R4-03 attribution; previous objectives stack pushed; Top priorities + Next actions reframed (R4-06 UW redesign now #2, R4-07 boss-drop PS now #3); Last run rotated.

- Open questions: none.

- Follow-ups created: commit R4-03 + push branch + open PR `feat(workshop): Rapid Fire upgrade (R4-03)` against `main`. Once merged, cut `feat/R4-06-uw-paths-auto-trigger` for the UW redesign — the largest single Wave 2 sub-plan (~80 new tests, schema migration v9→v10, ADR-0008).
...
## 2026-06-02 — V1X-09 Phase 3 (slice 5): UW lifecycle timers → Simulation

- Goal: continue V1X-09 Phase 3 with the UW lifecycle slice — extract the pure UW cooldown + effect-duration timer state machine + auto-trigger readiness predicate from `GameEngine.updateUWs` into the pure-domain `Simulation`. Scoped to the genuinely-pure timer arithmetic; the presentation-coupled activation effects (sound, visuals, enemy damage, stat mutation) deliberately stay in the engine (same boundary as the cash-economy slice, where the reward *formulas* stayed behind).

- Context preflight: read START_HERE / STATE / CONSTRAINTS / ADR-0012 / latest RUN_LOG; `git status` clean, `git log` confirmed `main` at `2631dc8` (post-PR-#96 doc sync). Read `Simulation` (slices 1–4 pattern), full `GameEngine` (inventoried the UW lifecycle: `UWState` data class + `uwStates` + `chronoActive`/`chronoSlowFactor`/`goldenZigActive`/`preGoldenStats`/`fortuneMultiplier` + `initUWs`/`activateUW`/`updateUWs`/`resetUWCooldowns`), `SimulationTest` (JUnit-5 idiom). Grepped every `uwStates`/`UWState`/`activateUW`/`updateUWs`/`initUWs` reference: consumers are `BattleViewModel` (`eng.uwStates.map` for the UI cooldown ring + `engine.initUWs`) and `GameEngineTest` (`eng.uwStates[0].cooldownRemaining` + `initUWs`+`activateUW`). Decision: keep `UWState` + `uwStates` + the whole public UW surface byte-identical on the engine, extract only the pure timer arithmetic as `Simulation` helpers the engine calls in-place — zero public-API churn, consistent with prior slices.

- Branch: `feat/V1X-09-phase3-uw-lifecycle` from `main`.

- Source changes:
  - **`domain/battle/engine/Simulation.kt`** — added a `// --- UW lifecycle timing ---` section: `UWTimerAdvance` data class (`cooldownRemaining`, `effectTimeRemaining`, `effectWasActive`, `justExpired`); `advanceUWTimers(cooldownRemaining, effectTimeRemaining, deltaTime)` (cooldown decremented + `coerceAtLeast(0f)` only when > 0; effect duration decremented only while `effectWasActive`, clamped to exactly 0 + `justExpired = true` on the crossing frame); `isUWReadyToFire(cooldownRemaining, effectTimeRemaining) = cooldownRemaining <= 0f && effectTimeRemaining <= 0f`. No class-KDoc edit (consistent with the collision/entity-tick slices, which documented at method level only).
  - **`GameEngine.updateUWs`** — the per-UW timer block now calls `simulation.advanceUWTimers(...)`, applies `cooldownRemaining`/`effectTimeRemaining` back onto the `UWState`, gates the one-shot CHRONO/GOLDEN expiry `when` on `timers.justExpired` and the ongoing BLACK_HOLE/POISON `when` on `timers.effectWasActive`. This preserves the EXACT pre-extraction ordering — original code ran the ongoing `when` inside `if (effectTimeRemaining > 0f)` (== `effectWasActive`), after the expiry check, so on the crossing frame BOTH expiry + one final ongoing tick run; my structure (`if (justExpired) { expiry }; if (effectWasActive) { ongoing }`, justExpired ⟹ effectWasActive) is identical. The auto-trigger gate swaps `uw.cooldownRemaining <= 0f && uw.effectTimeRemaining <= 0f` for `simulation.isUWReadyToFire(...)`. `activateUW` (sound/fx/enemy-damage/stat-mutation/chrono+golden flag writes) + `UWState` + `uwStates` + `initUWs` + `resetUWCooldowns` untouched.

- Tests: **`SimulationTest.kt`** (+7 pure-JVM) — 4 `advanceUWTimers` (cooldown decrement+clamp-at-0 / zero-cooldown+inactive-effect untouched / active countdown without expiry / `justExpired`+clamp on the crossing frame with `effectWasActive` still true) + 3 `isUWReadyToFire` (true off-cooldown-and-idle / false on cooldown / false mid-effect).

- Verification: `./run-gradle.sh testDebugUnitTest assembleDebug` BUILD SUCCESSFUL; test-results XML sum = 864 (857 → 864, exactly +7); `SimulationTest` now 33. The `GameEngineTest` UW + chrono + GOLDEN regression guards (`activateGoldenZigForTest`, `invokeUpdateUWs`, chrono-slow comparator) still pass — behaviour identical. No behaviour change.

- ADR: ADR-0012 — added the **UW lifecycle timers (2026-06-02)** per-slice bullet; trimmed "UW lifecycle" from the Remaining-slices line (now just the `SimulationEvent` flow).

- Doc-sync per agent protocol PR Task-List Convention: AGENTS.md (coverage 857 → 864 ×2 + V1X status slices 1–4 → 1–5 + slice-5 coverage clause), CHANGELOG.md (new `[Unreleased]` entry), `.kiro/steering/source-files.md` (Simulation.kt slices 1–5 + slice-5 sentence; SimulationTest.kt 26 → 33 + slice-5 description), `.kiro/steering/structure.md` (engine dir note + UW lifecycle timers), README.md (status banner + `# Unit tests (864 JVM tests)`), ADR-0012, STATE.md (this rotation), RUN_LOG.md (this entry). No tech.md / database-schema.md change.
...
## 2026-06-04 — V1X-13 i18n phase 1 (start): notification string extraction (branch feat/V1X-13-i18n-notifications)

- Goal: begin V1X-13 (i18n string extraction). User picked the V1X dev track over the closed-track soak / cloud-save, then chose the smallest first slice — notification strings — to land as a self-contained PR.
- Context preflight: read STATE / V1X roadmap; confirmed both remaining specced V1X items (V1X-12 cloud save, V1X-13 i18n) unstarted (`strings.xml` = `app_name` only; no `domain/Strings` seam; no `lint {}` block). Picked V1X-13 because it is fully buildable + verifiable in this environment (V1X-12 needs Play Games SDK + Play Console + a device).
- `git status` clean on `main` at `437aa84` (note: `main` had advanced past STATE's last-written head — PR #107 release-notes automation + PR #108 versionCode 17→18 / versionName 1.0.0→1.0.2 already merged). Branch `feat/V1X-13-i18n-notifications` from `main`.
- Scope decision: notification surface first because the 3 managers already hold a `Context`, so `context.getString(...)` needs **no** new abstraction and stays clear of the fragile battle/effects engine. The `domain/Strings` Hilt seam (for engine-internal floating-text strings that can't read `Context`), the battle + workshop screen surfaces, and the `HardcodedText` lint-as-error guard (must come last, or it fails the build on not-yet-migrated screens) are all deferred to later phase-1 PRs.
- Source: `res/values/strings.xml` +16 notification resources (6 step / 3 supply / 7 milestone). `StepNotificationManager` / `SupplyDropNotificationManager` / `MilestoneNotificationManager` now read all channel names/descriptions, titles, content, and action labels via `context.getString(...)`; each gained `import com.whitefang.stepsofbabylon.R`. Positional format args (`%1$d` / `%2$s`) so translations can reorder; step notification title reuses `R.string.app_name`; `SupplyDrop.trigger.message` left as-is (domain-sourced). Diff: 4 files, +48 / −17.
- Verification: `./run-gradle.sh testDebugUnitTest assembleDebug` BUILD SUCCESSFUL in 2m48s; 867 JVM unchanged (pure refactor, no new tests — no behaviour change, matches the V1X-04 asset-swap precedent of zero test delta). Lint `HardcodedText`-as-error guard intentionally NOT added this slice.
- Doc sync per agent protocol PR Task-List Convention: CHANGELOG.md (new `[Unreleased]` entry), `.kiro/steering/source-files.md` (strings.xml entry expanded), STATE.md (this rotation — current-objective bullet + Last-run line), RUN_LOG.md (this entry). No AGENTS test-count change (867 unchanged); no README / tech.md / structure.md / database-schema.md change.
- Next: commit + push + open PR for this slice. Then later phase-1 PRs: `domain/Strings` seam (engine-internal strings), battle + workshop surfaces, finish with the `HardcodedText` lint guard.
- Memory updated: STATE ✅ / RUN_LOG ✅

## 2026-06-04 — V1X-13 i18n phase 1 (cont.): domain/Strings seam (branch feat/V1X-13-i18n-strings-seam)

- Goal: second i18n slice — route the engine-internal battle floating-text strings through a localization seam. These are the hard case (slice 1 did notifications): emitted by `GameEngine` (a plain, manually-constructed class deliberately Context-free so `GameEngineTest` is pure-JVM) and `BattleViewModel`.
- Branch `feat/V1X-13-i18n-strings-seam` from `main` (clean, `main` at `ee29439` post-#109).
- Design (ADR-0014): `domain/Strings` pure-Kotlin interface (5 methods: healHp / rapidFireBurst / cashReward / stepReward / powerStoneReward — no Android imports, passes `DomainPurityTest`) + `data/AndroidStrings` impl (`Context.getString` over 5 new `fx_*` resources). Chose nullable `var strings: Strings?` on `GameEngine` with a **byte-identical literal fallback** (mirrors the `soundManager`/`effectEngine` nullable-collaborator pattern) so existing `GameEngineTest` (R3-02 lifesteal `+X HP`, R4-07 boss tests) + `BattleViewModel` step-reward tests stay green with ZERO churn and no fake/Robolectric. `GameSurfaceView` constructs `AndroidStrings(context)` directly (like it builds `SoundManager`) and sets `engine.strings`; `BattleViewModel` reads `engine.strings` off the engine reference it already holds in `handleSimulationEvent` — so no Hilt binding and no VM constructor change.
- Source: NEW `domain/Strings.kt`, NEW `data/AndroidStrings.kt`, `strings.xml` +5 `fx_*`; `GameEngine` (+import, +`var strings`, 4 FloatingText sites → `strings?.X(...) ?: "<literal>"`); `GameSurfaceView` (+import, +`engine.strings = AndroidStrings(context)`); `BattleViewModel` (2 FloatingText sites → `engine.strings?.X(...) ?: "<literal>"`). Domain home (over presentation) chosen so future pure-domain `Simulation` can reuse the seam.
- Verification: `./run-gradle.sh testDebugUnitTest assembleDebug` BUILD SUCCESSFUL in 34s; 867 JVM unchanged (pure refactor; `DomainPurityTest` green confirms `domain/Strings` is Android-free).
...
## 2026-06-04 — V1X-13 i18n phase 1 (cont.): battle + workshop Compose surfaces (branch feat/V1X-13-i18n-compose-surfaces)

- Goal: third V1X-13 slice — migrate the battle + workshop Jetpack Compose surfaces off hardcoded string literals onto `stringResource`. User said "proceed" after I proposed this as the next step to finish phase 1.
- Context preflight: read STATE / the V1X-13 plan section / ADR-0014 / the 7 target Compose files. `git status` clean on `main` at `0e7f907`. Branch `feat/V1X-13-i18n-compose-surfaces` from `main`.
- Scope: the 6 plan-named files (`WorkshopScreen`, `UpgradeCard`, `BattleScreen`, `PostRoundOverlay`, `PauseOverlay`, `InRoundUpgradeMenu`) + `BiomeTransitionOverlay` (completes the visible battle-overlay surface). `UltimateWeaponBar` skipped (only dynamic-number `Text` + content descriptions; no static literal). `WaveAnnouncement` / `GameEngine` Canvas announcement text is engine-internal (not Compose) → a later domain/Strings extension / phase 2. All other screens (Store / Settings / Cards / Home / Economy / Labs / Missions / Stats / Supplies / UltimateWeapon / Help) are phase 2 (v1.3).
- Source: `res/values/strings.xml` +~45 resources (workshop_*, upgrade_*, battle_*, cash_amount, steps_earned_banner, inround_*, pause_*, action_resume/continue, biome_*, postround_*). 7 Compose files migrated to `stringResource(...)`; `PostRoundOverlay` share text + chooser via `context.getString(...)` inside the share `onClick` (non-composable lambda). Content descriptions hoisted out of `semantics {}` blocks to composable scope (`stepsDesc` / `pauseDesc` / `upgradesDesc`) since `semantics {}` is not a composable context. Field types verified all `%d`-compatible (Long/Int); `cashMultiplier` passed `.toString()` → `%1$s`. Pure glyphs (▶ / ⏸ / ✕) + dynamic enum/number text left literal.
- Verification: `./run-gradle.sh testDebugUnitTest assembleDebug` BUILD SUCCESSFUL in 29s; 867 JVM unchanged (pure refactor, no behaviour change). Residual-`Text("…literal…")` grep across `presentation/` confirms 0 leftovers in the 7 targeted files.
- Doc sync per agent protocol PR Task-List Convention: CHANGELOG.md (new `[Unreleased]` entry), `.kiro/steering/source-files.md` (strings.xml entry + 7 file entries), AGENTS.md (Plan V1X status bullet), README.md (status banner), STATE.md (this rotation), RUN_LOG.md (this entry). No test-count change (867 unchanged); no tech / structure / database-schema change.
- Next: commit + push + open PR (pending user confirmation). Then the FINAL phase-1 PR: the `HardcodedText` lint-as-error guard + the `docs/architecture.md` policy note.
- CI follow-up: committed `ac8a9e8`, pushed, opened **PR #111**. The `build-and-test` gate FAILED on 2 `LocalContextGetResourceValueCall` lint errors (`PostRoundOverlay.kt` 131/136 — the `context.getString(...)` calls I added in the share `onClick`); the `connected` instrumented lane passed. Root cause: I verified locally with `testDebugUnitTest assembleDebug` but the CI gate also runs `lintDebug` (which I skipped). Fix: hoisted the share text + chooser title to `stringResource(...)` at composable scope, leaving only `startActivity` in the lambda. Re-ran the FULL gate `testDebugUnitTest lintDebug assembleDebug` locally — BUILD SUCCESSFUL, 867 unchanged. Lesson: always run `lintDebug` (the full CI gate command) before pushing, not just tests + assemble.
- Merged: PR #111 merged via merge commit `ba8b1bb` after both required checks (`build-and-test` + `connected`) went green on the re-run; local `main` fast-forwarded `0e7f907..ba8b1bb`; feature branch deleted. Post-merge doc sync (STATE + this RUN_LOG) committed directly to `main` per the post-merge precedent.
- Memory updated: STATE ✅ / RUN_LOG ✅

## 2026-06-04 — V1X-13 i18n phase 1 (final): HardcodedText lint-as-error guard; PHASE 1 COMPLETE (branch feat/V1X-13-i18n-lint-guard)

- Goal: the final V1X-13 phase-1 slice — add the `HardcodedText` lint-as-error guard (ADR-0014 decision #4, "added last once every phase-1 surface is migrated"). User said "yes" to starting it.
- Context preflight: read `app/build.gradle.kts` (no `lint {}` block) + enumerated XML resources (one layout: `widget_step_counter.xml`). Branch `feat/V1X-13-i18n-lint-guard` from `main` (`194da38`, clean, post-#111-merge).
- Plan-snippet correction: ADR-0014 / the plan wrote `lint { warningsAsErrors += "HardcodedText" }`, but `warningsAsErrors` is a **Boolean** DSL property (promotes ALL warnings to errors). To target one check the correct API is the `error` `MutableSet` → used `lint { error += "HardcodedText" }`.
- Source: `app/build.gradle.kts` +`lint { error += "HardcodedText" }` block (after `testOptions`, with a comment documenting the XML-only limitation). `res/layout/widget_step_counter.xml` — 3 `HardcodedText` violations fixed: static title → `@string/app_name`; `widget_daily_steps` + `widget_balance` (set at runtime via `RemoteViews` by `WidgetUpdateHelper`) → design-time `tools:text` (+`xmlns:tools`), so no `android:text` literal remains.
- **Empirical finding (resolves the plan's "investigate Lint's HardcodedText" note):** with the guard enabled, `./run-gradle.sh lintDebug` is BUILD SUCCESSFUL — despite ~110 hardcoded Compose `Text("literal")` strings still present on the phase-2 screens (Store / Settings / Cards / Home / Economy / Labs / Missions / Stats / Supplies / UltimateWeapon / Help). If `HardcodedText` inspected Compose, those would all fail. Conclusion: `HardcodedText` is **XML-only** and does NOT flag Compose `Text()`. The guard concretely protects XML resources only (today: the widget layout). Compose string discipline is held by the phase-1 migration + review; a dedicated Compose rule (custom lint / Detekt-compose / a dependency-free JVM guard test à la `DomainPurityTest`) is a deferred option — intentionally not added to avoid a new dependency.
- Docs: new "Internationalization (i18n)" section in `docs/architecture.md` (phase 1/2/3 split + `domain/Strings` seam + the lint policy + the empirically-verified Compose limitation + the `warningsAsErrors`-is-a-Boolean gotcha).
- Verification: `./run-gradle.sh testDebugUnitTest lintDebug assembleDebug` BUILD SUCCESSFUL; 867 JVM unchanged (no source/test behaviour change — build-config + XML + docs only).
...
- Merged: PR #114 merged via merge commit `b86e043` 2026-06-10. Branch protection is strict (head must be up to date) — `main` had advanced by docs-only PR #113 (security-audit report), so merged `main` into the branch (`5d8ea19`, no conflicts), re-ran CI (both `build-and-test` + `connected` green), then merged + deleted branch. Local `main` synced to `b86e043`. Post-merge doc sync (STATE + this RUN_LOG) committed directly to `main`.
- Memory updated: STATE ✅ / RUN_LOG ✅

## 2026-06-10 — Sort Dependabot alerts: scope dependency graph to runtime classpaths

- User: "sort the dependabot stuff" (the 32 alerts the V1X-14 push warned about). Triaged via `gh api .../dependabot/alerts?state=open`: 32 open (14 high / 15 medium / 3 low).
- **Root-cause finding:** every alert's `manifest_path` is `settings.gradle.kts`. The package set (io.netty:netty-codec-http ×10, netty-codec/http2/handler/common/handler-proxy, protobuf-java/kotlin, jose4j, bouncycastle bcprov/bcpkix, jdom2, commons-lang3, httpclient, guava) is the Google-API-client transitive tree behind the Play Publisher plugin + AGP/bundletool. The `dependency-submission.yml` action (`gradle/actions/dependency-submission`) submits the ENTIRE Gradle graph by default — including the buildscript/plugin `classpath` config — so build-tool-only deps landed in GitHub's security graph.
- **Verification:** `./run-gradle.sh -q :app:dependencies --configuration releaseRuntimeClasspath` + grep — of all flagged packages, ONLY `com.google.guava:guava:31.1-android` is in the shipped runtime classpath (transitive via kotlinx-coroutines-guava etc.). netty/protobuf-java/jose4j/bouncycastle/jdom2/commons-lang3/httpclient: ABSENT (build-only). So 30 of 32 alerts are non-shipping false-signal; 2 (guava, medium) are real-but-transitive.
- **Fix (branch `feat/32-scope-dependency-graph`):** added `env: DEPENDENCY_GRAPH_INCLUDE_CONFIGURATIONS: ".*RuntimeClasspath"` to the dependency-submission step + a rationale header comment. Scopes the submitted graph to release/debug/test runtime classpaths, excluding the buildscript `classpath`. On the next push:main submission after merge, the 30 build-only deps leave the graph → their alerts auto-dismiss; guava remains (correct). YAML validated via `ruby -ryaml` (env nested on last step). Workflow-only — no app code/test/schema change. NB `dependency-submission.yml` runs on push:main only, so it does NOT run on the PR — the alert drop is observable post-merge.
- Doc sync: CHANGELOG.md (new `[Unreleased]` Security entry), `.kiro/steering/tech.md` (dependency-submission line notes the scoping), STATE.md (current objective), RUN_LOG.md (this entry). No test-count change (867).
- **Residual / recommended follow-up:** the 2 guava alerts are genuinely shipped (medium). Separate PR: force transitive guava ≥32.0.0-android (e.g. a dependency constraint in app/build.gradle.kts) with the full `testDebugUnitTest lintDebug assembleDebug` gate — kept out of this CI-config PR since it's a shipped-code change with its own (small) runtime risk.
- Next: commit + push + open PR; after merge, confirm the 30 alerts auto-dismiss on the dependency-submission run, then decide on the guava bump.
- Memory updated: STATE ✅ / RUN_LOG ✅
- Merged: PR #115 via merge commit `3f04cd3` 2026-06-10 (both CI checks green; branch was based on current main, no update needed). The merge's push:main triggered the dependency-submission run with the new scope; GitHub reconciled the graph: **32 → 4 open alerts.** The 28 build-tool-plugin false-positives (netty ×19, protobuf-java/kotlin, jose4j, jdom2, httpclient, commons-lang3, netty-handler-proxy) auto-dismissed.
- **Correction to the pre-merge prediction (which said "guava only / 2 remain"):** the actual residual is **4**, because the scope `.*RuntimeClasspath` includes TEST runtime, and Robolectric (debugUnitTestRuntimeClasspath) transitively pulls `org.bouncycastle:bcprov-jdk18on:1.78.1`. Residual: guava ×2 (#4 medium / #5 low — `releaseRuntimeClasspath`, genuinely shipped) + bcprov ×2 (#22 medium / #23 high — Robolectric test-only, NOT in releaseRuntimeClasspath, verified clean). bcprov confirmed test-only via `:app:dependencies --configuration releaseRuntimeClasspath` (empty) vs `debugUnitTestRuntimeClasspath` (bcprov under org.robolectric:* 4.14.1).
- **Open decisions (handed to user):** (1) optionally tighten scope to `releaseRuntimeClasspath` only → security graph = production exposure → leaves guava-only and drops the un-actionable test-only bcprov high; (2) bump transitive guava ≥32.0.0-android (shipped fix; needs full test gate; separate PR); (3) bcprov is only fixable by a Robolectric version bump (test-only, low urgency) or dismiss as not-shipped.
- Memory updated: STATE ✅ / RUN_LOG ✅

## 2026-06-10 — Plan 32 security follow-ups: release-only dependency graph + patched guava

- User: "go for it" on the two recommended follow-ups after the 32→4 dependency-graph scoping. Branch `feat/32-security-followups` from main (67da334).
- **Change A — tighten scope to release-only.** `dependency-submission.yml` env `DEPENDENCY_GRAPH_INCLUDE_CONFIGURATIONS` changed from `.*RuntimeClasspath` to a regex anchored on `releaseRuntimeClasspath` only. Rationale: the broader scope still surfaced test-only deps — `bcprov-jdk18on:1.78.1` (high #23 / medium #22) pulled by Robolectric in `debugUnitTestRuntimeClasspath`, which never ships. Anchoring to the production AAB runtime classpath makes the security graph = production exposure; #22/#23 leave the submitted graph and auto-dismiss on the next push:main submission.
- **Change B — force transitive guava to patched version.** guava is NOT a direct dep (transitive via kotlinx-coroutines-guava + Play Services at 31.1-android; CVE-2023-2976 / CVE-2020-8908). Added `guava = "33.4.0-android"` to libs.versions.toml [versions] + a `guava` [libraries] entry, and a `constraints { implementation(libs.guava) { because(...) } }` block in app/build.gradle.kts. Verified `:app:dependencies --configuration releaseRuntimeClasspath` now resolves `31.1-android -> 33.4.0-android`. Clears #4/#5.
- Verification: `./run-gradle.sh testDebugUnitTest lintDebug assembleDebug` BUILD SUCCESSFUL (3m); 867 JVM unchanged (CI-config + transitive-version constraint, no source/test behaviour change).
- Tooling note: `edit_file` mangles a literal `$` in newText (it duplicated the whole file at the `$` in `^releaseRuntimeClasspath$`), corrupting dependency-submission.yml first (fixed via write_file) then CHANGELOG.md + tech.md (restored via `git checkout --` then re-edited with the regex described in prose, no literal `$`). Lesson: avoid literal `$` in edit_file content; use write_file or prose.
- Doc sync: CHANGELOG.md (new [Unreleased] Security entry), tech.md (dependency-submission line → release-only), STATE.md (current objective), RUN_LOG.md (this entry). No test-count change (867); no schema/source change.
- Next: commit + push + open PR; watch CI (ci.yml + connected); merge; then confirm the post-merge push:main dependency-submission run reconciles open alerts to 0.
- Merged: PR #116 via merge commit `e494c11` 2026-06-10 (both CI checks green: build-and-test 6m15s, connected 6m53s). The post-merge push:main dependency-submission run completed and the graph reconciled: **open Dependabot alerts 4 → 0 (verified via API).** guava ×2 cleared by the 33.4.0-android bump; bcprov ×2 dropped by the release-only scope. Dependabot dashboard now clean (was 32 at session start).
- Memory updated: STATE ✅ / RUN_LOG ✅
