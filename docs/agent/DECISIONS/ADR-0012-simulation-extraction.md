...
Remaining slices: collision migration (the `CollisionSystem` callbacks are still typed to the concrete presentation entities `ProjectileEntity` / `EnemyEntity` / `EnemyProjectileEntity`), UW lifecycle, and the `SimulationEvent` flow replacing the `@Volatile` callbacks.
