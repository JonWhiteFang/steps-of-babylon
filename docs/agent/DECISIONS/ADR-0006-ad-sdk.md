...
| Q5 | **COPPA / child-directed flag.** Mark the AdMob app as child-directed to stay COPPA-safe? | **No.** The game targets adults. Play Store age rating + `minSdk 34` (Android 14) handle the kid-safety surface. Marking child-directed would suppress personalised ads for all users, reducing eCPM for no compliance benefit. **(Amended 2026-06-20, #241 / ADR-0032 — stance RETAINED, REFINED:** still no child-directed flag and no age gate, but the previously-absent `RequestConfiguration` is now set before the first ad request with `maxAdContentRating = PG` only. The 13+ adult positioning is unchanged; this just bounds the served-ad content rating to match the "not directed to children" policy text. See ADR-0032.) |
