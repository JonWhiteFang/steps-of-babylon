...
- **Flat per-enemy-type rewards.** No wave scaling, no multiplication by Cash Bonus upgrades or the Golden Ziggurat UW. This keeps daily yield predictable for the cap logic and anti-cheat auditing, and stops late-wave runs from minting Steps at an accelerating rate. (R4-01 amendment 2026-05-23: the original wording also called out "Fortune overdrive" as a non-multiplier; that constraint became vacuously true when R4-01 deleted Step Overdrive entirely. The flat-rate invariant remains — R4-06 will redesign GOLDEN_ZIGGURAT but the no-multiplier rule still holds for battle Steps.)
...
- **2026-05-23 (R4-01):** Step Overdrive deleted entirely as part of Plan R4 Wave 1. The Rationale's "no Fortune-overdrive multiplier" constraint becomes vacuously true — there is no Fortune overdrive to gate against. The flat-rate invariant for battle Steps is unchanged: rewards remain per-enemy-type only, with no in-round multiplier from any source. R4-06 will redesign the Golden Ziggurat UW with per-path upgrades; the rule that GOLDEN does not multiply battle Steps still holds (its `fortuneMultiplier` only affects Cash, never Steps).
