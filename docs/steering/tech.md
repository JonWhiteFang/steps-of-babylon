...
- `osv-scan` — weekly + push:main + manual. **`allow_failure: true`**, deliberately *not* `|| true`: findings turn the job yellow (visible, non-blocking) while a genuinely *broken scanner* still shows red. SARIF artifact only — GitHub's Code-Scanning dashboard has no GitLab equivalent on our tier (ADR-0044), and the dependency-submission API is likewise retired.
