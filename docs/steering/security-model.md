...
- **gitleaks CI gate** (the `gitleaks` job in `.gitlab-ci.yml` + `.gitleaks.toml`) — repo-committed,
  runs on every MR and push to `main` over full history (`GIT_DEPTH: 0` — a shallow clone silently
  weakens it). Extends the default ruleset with rules
