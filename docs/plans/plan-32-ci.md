...
- **Dependency graph** — `gradle/actions/dependency-submission@<sha>` on `push` to `main` so GitHub security alerts see the resolved Gradle graph (scoped to `releaseRuntimeClasspath` — the shipped graph only).
- **Supply-chain scan (#262 L77, post-authoring)** — `osv-scan.yml` runs `google/osv-scanner-action`'s reusable workflow (SHA-pinned) over the FULL dependency set (build/test-time too, which the release-only dependency-graph above misses) → Code Scanning tab. **Non-gating** (`fail-on-vuln: false`), weekly + on `main`.
- **Coverage (#218, post-authoring)** — Kover (`:app`, `org.jetbrains.kotlinx.kover` 0.9.8) emits a coverage report on the PR gate as an **informational, non-gating** artifact (no threshold). Adding it required regenerating `gradle/verification-metadata.xml` (strict verify, #256) + extending the documented regen command with the Kover tasks.
