...
- **Q1 FAILED** → the instrumented lane **cannot** use shared runners. **OWNER DECISION 2026-07-23: demote the instrumented suite to LOCAL-ONLY** — drop it from the CI gate; the 9 instrumented tests run on a device before releases (as the plan's fallback allows). No self-hosted runner, no `«RUNNER_TAG»`, no Firebase Test Lab. **Accepted regression:** instrumented tests no longer block MRs on GitLab (they were a blocking lane on GitHub) — record in ADR-0044. The self-hosted-runner + Firebase options were considered and declined (self-hosting = running public-MR code on personal hardware + hardening burden; Firebase = added GCP integration + per-run cost).
