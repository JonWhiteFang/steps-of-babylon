...
- [x] Privacy policy written (`site/index.md` — canonical TEXT; the hosted page users and Play reach is served
      off-forge by the website deployment, and the `pages` job in `.gitlab-ci.yml` publishes `site/` only — ADR-0044)
...
- [x] Privacy policy hosted at a public URL — <https://jonwhitefang.github.io/steps-of-babylon/>
- [ ] **Relocation pending (ADR-0044):** once <https://jonwhitefang.uk/legal/steps-of-babylon-privacy/> is
      confirmed live, the `PRIVACY_POLICY_URL` constant + both locale strings switch to it and the Play
      fields are updated. Until then the github.io URL is the authoritative one, and it keeps serving a full
      copy afterwards for already-installed builds.
