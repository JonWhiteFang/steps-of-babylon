...
- Add a small standalone Gradle task `generateLicenseReport` in **`app/build.gradle.kts`** OR a dedicated
  ...
  - **Preferred low-risk implementation:** a **committed, hand-curated `res/raw` text file** listing the
    direct third-party runtime dependencies + their licenses. A regeneration task derives the *coordinate
    list* from `gradle/libs.versions.toml` (the single source); the **license bodies are a committed
    hand-curated template** (Apache-2.0 full text once + per-dep copyright lines) — `libs.versions.toml`
    carries only group/name/version, no SPDX id or license text, so the task cannot auto-produce the notice
    text (re-review fix #5: scope the drift-mitigation honestly — regeneration catches added/removed deps,
    NOT license-text changes). **Do NOT** pull a third-party license-scanner plugin (that reintroduces the
    dependency-weight problem this option exists to avoid).
  - **§4(d) coverage MUST enumerate the FULL direct `implementation` set** (re-review fix #4 — the earlier
    short list under-enumerated). From `libs.versions.toml [libraries]` + `app/build.gradle.kts:304-416`,
    the direct **shipping** deps are: Compose (BOM + ui/graphics/material3/material-icons — Apache-2.0),
    `coroutines-android` (Apache-2.0), `core-ktx`, `activity-compose`, `lifecycle-viewmodel-compose`,
    `lifecycle-runtime-compose`, `navigation-compose` (all Apache-2.0), Room (`room-runtime`/`room-ktx` —
    Apache-2.0), Hilt (`hilt-android`, `hilt-work`, `hilt-navigation-compose` — Apache-2.0), WorkManager
    (Apache-2.0), `androidx.profileinstaller` (Apache-2.0 — **ships in the AAB; was omitted before**),
    Health Connect (Apache-2.0), `sqlite-ktx` (Apache-2.0), **SQLCipher** (`net.zetetic:sqlcipher-android`
    — **BSD 3-clause with the Zetetic copyright + redistribution clause**, NOT loosely "BSD-style"),
    `billing-ktx` (Android SDK license), `play-services-ads` + `play-services-base` (Android SDK license),
    **`user-messaging-platform`** (`com.google.android.ump` — Android SDK license; **was omitted before**).
    **Exclude** `testImplementation`/`debugImplementation`/`androidTestImplementation`/`kspAndroidTest`
    (not shipped) and the `guava` **constraint** (a version floor, not a direct dep — though guava DOES ship
    transitively; list it if the notice aims to cover transitive Apache-2.0 too, but the §4(d) obligation is
    satisfied by covering the direct redistributed set).
  - The generated file lands as a **`res/raw/oss_notices.txt`** (raw resource, referenced by `R.raw.*`), so
    R8 resource-shrinking keeps it (a static `R.raw` id reference is a keep root — **confirmed precedent:
    the 9 existing `.ogg` files already in `res/raw/` ship via exactly this pattern**; the dir already
    exists — only the `oss_notices.txt` file is new, re-review fix #1).
...
- `HelpScreen` is a plain scrollable `Column` of `HelpSection(title, body)` (`HelpScreen.kt:20-77`). Add a
  ...
  - Read the raw resource once at composition, **closing the stream** (re-review fix #3 — do NOT leave the
    `InputStream` unclosed):
    ```kotlin
    val context = LocalContext.current
    val ossNotices = remember {
        context.resources.openRawResource(R.raw.oss_notices).bufferedReader().use { it.readText() }
    }
    ```
    New imports required (neither currently in `HelpScreen.kt`, both pure Compose/AndroidX-UI — no
    `data.local`, so `PresentationPurityTest` is unaffected): `androidx.compose.ui.platform.LocalContext`
    and `androidx.compose.runtime.remember`. Render inside the existing `HelpSection(title, body)` idiom
    (`HelpScreen.kt:79-85`) — a title `stringResource` + the loaded `ossNotices` String as the body.
...
## Lean re-review of the static-asset #377 (PR-3) — DONE (2026-07-03)
The multi-agent gate reviewed the plugin-route #377; the static-asset PR-3 is a new artifact, so it got a
**lean single-agent grounding re-review**. Verdict: **PR-3 largely sound** — the R8-keep, task-placement
vs #124, `ComposeHardcodedStringTest` and `DeepLinkRoutingTest` non-regression claims all confirmed correct
against the real files. Three fixes applied above: **(4, major)** §4(d) must enumerate the FULL direct
`implementation` set (earlier list omitted `user-messaging-platform`, `profileinstaller`, the AndroidX
family) + cite SQLCipher's exact BSD-3-clause+Zetetic license, not "BSD-style"; **(3, minor)** the raw-read
idiom must `.use { }`-close the stream + names the two new imports; **(5, nit)** license *bodies* are
hand-curated (the version catalog has no license text) — regeneration catches added/removed deps, not
license-text changes. **(1, minor)** `res/raw/` already exists (only the file is new). PR-1 and PR-2 were
fully reviewed in the multi-agent gate and may proceed immediately.
