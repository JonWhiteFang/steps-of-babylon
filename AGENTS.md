...
- [ ] **Plan 32: CI/CD Pipeline (GitHub Actions)** *(authored 2026-06-03 — `docs/plans/plan-32-ci.md` + ADR-0018. Three lanes: `ci.yml` PR gate (lint + unit + assembleDebug + schema-drift), `instrumented.yml` emulator suite (blocking-on-PR + nightly), `release.yml` signed bundleRelease → Play internal track on a `v*` tag. SHA-pinned actions + Dependabot. Implemented on branch `feat/32-ci-pipeline` (PR pending); local gate verified green (867 tests — also surfaced + fixed a latent `lintDebug` error). Repo secrets + `release` environment + branch protection are post-merge setup.)*
