...
domain/battle/engine/Simulation.kt # V1X-09 Phase 3 (ADR-0012, slice 1). Pure-domain owner of in-round simulation state. First slice: the cash economy — @Volatile cash / totalCashEarned (private set) + reset / creditCash (credits both balances) / applyInterest (cash only, min(level×0.5%,10%), excluded from lifetime) / spend (insufficient-balance guard). GameEngine holds a Simulation and delegates its cash / totalCashEarned / spendCash public surface here; per-kill + wave-complete reward *formulas* stay in GameEngine and call creditCash / applyInterest. No Android imports.
```
...
domain/battle/engine/SimulationTest.kt             # 9 pure-JVM tests for Simulation (V1X-09 Phase 3, ADR-0012, slice 1 — cash economy): creditCash accumulates both balances + sums across credits + ignores non-positive; applyInterest adds to cash only (not lifetime) + caps at 10 % + level-0 no-op; spend deducts when affordable + rejects/leaves balance untouched when insufficient; reset zeroes both. No Robolectric.
