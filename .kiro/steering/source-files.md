...
domain/battle/entity/EntityProtocol.kt # V1X-09 Phase 3 (ADR-0012, slice 3). Pure-domain entity seam (no Android imports): `val isAlive` + `val isChronoSlowable` (default false) + `fun update(deltaTime)`. Lets `Simulation.tickEntities` iterate battle entities without touching Canvas. Implemented by presentation `Entity`; `EnemyEntity` overrides `isChronoSlowable = true`.
...
domain/battle/engine/SimulationTest.kt             # 20 pure-JVM tests for Simulation (V1X-09 Phase 3, ADR-0012, slices 1–3). Slice 1 cash economy (9): creditCash accumulates both balances + sums + ignores non-positive; applyInterest adds to cash only (not lifetime) + caps at 10 % + level-0 no-op; spend deducts when affordable + rejects/leaves balance untouched when insufficient; reset zeroes both. Slice 2 round-progress (7): tickElapsed accumulates the clock; recordEnemyKilled increments; creditSteps accumulates + ignores non-positive; hasWaveProgress false-on-fresh / true-after-tick / true-after-kill; reset zeroes the counters. Slice 3 tickEntities (4): scales dt for a chrono-slowable fake, full dt for a non-slowable fake, factor-1 no-op, mixed-list all-ticked-once (private FakeEntity impl of EntityProtocol). No Robolectric.
