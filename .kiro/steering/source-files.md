...
domain/battle/engine/SimulationTest.kt             # 16 pure-JVM tests for Simulation (V1X-09 Phase 3, ADR-0012, slices 1–2). Slice 1 cash economy (9): creditCash accumulates both balances + sums + ignores non-positive; applyInterest adds to cash only (not lifetime) + caps at 10 % + level-0 no-op; spend deducts when affordable + rejects/leaves balance untouched when insufficient; reset zeroes both. Slice 2 round-progress (7): tickElapsed accumulates the clock; recordEnemyKilled increments; creditSteps accumulates + ignores non-positive; hasWaveProgress false-on-fresh / true-after-tick / true-after-kill; reset zeroes the counters. No Robolectric.
