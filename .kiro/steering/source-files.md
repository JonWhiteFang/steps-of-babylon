...
presentation/battle/entities/ZigguratEntity.kt     # 5-layer ziggurat, nearest-enemy targeting, HP tracking. RO-08 / R4-01: stats is `var` with private setter + updateStats() called by GameEngine.applyStats; attackInterval / attackRange are computed properties reading the live stats so in-round upgrades + GOLDEN_ZIGGURAT damage buff propagate correctly mid-round. R4-01 dropped overdriveColor / overdriveProgress fields + the timer-bar render block + the timerBgPaint / timerFillPaint paints (used only by the timer bar).
