...
- **Version:** 1.0.0 (versionCode 17 — historical sequence v3 → v7 (2026-05-21, full {RO-12 + R3-01 through R3-04} bundle) → v9 (2026-05-23, Plan R4 Wave 1 bundle, smoke test PASSED) → v10 (2026-05-24, R4-07 boss-drop PS) → v11 (2026-05-24, Plan R4 complete + Quick Invest FAB removal, uploaded to closed track) → v12 (2026-05-25, #18 ad-rewarded card pack persistence fix) → v13 (2026-05-25 evening, #19 + #20 fix bundle, rejected by Play Console as "versionCode already used") → v14 (2026-05-26 early hours, same bundle as v13 with versionCode bumped 13 → 14, uploaded + smoke test PASSED 2026-05-26 ~05:35 BST) → v15 (2026-05-27, v14-soak hotfix bundle: PR #56 #53 + PR #57 #55 + PR #58 #54, versionCode 14 → 15, uploaded to closed track 2026-05-27 ~05:39 BST) → **v16 (2026-05-28, v15-soak hotfix bundle commit `0ecfe1d` fixing #59/#60/#61/#62/#63, versionCode 15 → 16, AAB built 2026-05-28 ~05:19 BST, uploaded by user)**.
...
Development follows a master plan with 38 entries (Plans 01–32, 10b, R, R2, R3, R4, and V1X). See `docs/plans/master-plan.md` for the full index, dependency graph, and status tracker.
...
| 32 | CI/CD Pipeline | GitHub Actions CI/CD. PR gate (lint + unit + assembleDebug + Room schema-drift guard), instrumented emulator suite (blocking-on-PR + nightly), release lane (signed bundleRelease → Play internal track on a `v*` tag). Supply-chain hardening: Dependabot, dependency-graph submission, SHA-pinned actions. ADR-0018. | Plan 29, Plan 30, Plan 31 |
...
- [x] **Plan R4: Remediation 4 — Internal Soak Feedback Bundle** ✓ *(All 8 sub-plans merged to `main` via PRs #9–#17. Wave 1 (no schema): R4-01 remove Step Overdrive, R4-02 + R4-02b Multishot/Bounce 4-level + Labs research path, R4-04 in-round upgrade button icon. Wave 2 (schema v9→10): R4-03 Rapid Fire upgrade, R4-06 UW auto-trigger + per-path upgrades (ADR-0008), R4-07 boss-drop Power Stones (ADR-0009). Wave 3 (schema v10→11): R4-08 cards copy-based 7-level progression (ADR-0010). Wave 4 (no schema): R4-05 Help screen. Quick Invest FAB removed per user request (commit `5e3530c`). Net test count delta 615 → 646 (R4 closure) → 656 post-#19/#20 fix bundle. AAB v9 → v10 → v11 (uploaded to closed track 2026-05-24) all built + signed; v12 (#18 ad-rewarded card pack persistence fix) → v13 (rejected by Play Console as "versionCode already used") → **v14 (Play-Console-rejection workaround, on-device smoke test PASSED 2026-05-26 ~05:35 BST)**.)*
- [ ] **Plan 31: Play Console & Store Publication** *(in closed-track soak — AAB v14 smoke test PASSED 2026-05-26; v15 uploaded 2026-05-27; v16 (V1X work bundle) built 2026-05-28. Closed-track ≥14-day window resumes from v14 effective 2026-05-26; earliest production-access application stays 2026-06-09. **Active gating activity is tester recruitment + soak.** Closed-track ≥14-day / ≥12-tester gate then production access application + Google review (1–3 days) + staged rollout + tag v1.0.0.)*
- [ ] **Plan V1X: v1.x Roadmap** *(execution in progress — 16 sub-plans merged to `main` 2026-05-28 across PRs #64–#79; V1X-08 instrumented tests completed 2026-05-29 across PRs #80–#83 (`BattleSurfaceLifecycleTest` + `DeepLinkIntentTest` shipped, `StoreIapFlowTest` formally deferred), and V1X-15b ENEMY_INTEL combat foundation merged via PR #84, with the ENEMY_INTEL L1/L5/L10 UI overlays following on a dedicated PR. Wave 1 complete (V1X-01/02/03), Wave 2 complete (V1X-04/05/06), Wave 3 partial (V1X-07/08/09/10/11), content/balance/docs partial (V1X-14/15/15b/16/17/18/19). 4 ADRs added: ADR-0012 simulation extraction, ADR-0015 STEP_MULTIPLIER asymptotic curve, ADR-0016 GPS reconciliation, ADR-0017 ENEMY_INTEL design. Test count 656 → 866 (+210 JVM) + 9 instrumented. V1X-15b ENEMY_INTEL combat + UI overlays complete and on-device verified 2026-05-29 (+2 %/lvl coefficient confirmed; ADR-0017 balance items resolved). **V1X-09 Phase 2 (entity extraction) COMPLETE** — all 5 battle entities now delegate simulation state to `domain/battle/entity`: `ProjectileState` (Projectile + EnemyProjectile), `OrbState` (Orb orbit/oscillation), `EnemyState` (Enemy movement + attack-cooldown), `ZigguratState` (ziggurat HP/regen + attack-cooldown + derived attackInterval/attackRange). Remaining substantial items: **V1X-09 Phase 3 (update-loop → `Simulation`) COMPLETE 2026-06-03** — all 6 slices lifted the in-round cash economy, the round-progress counters (`totalEnemiesKilled`/`totalStepsEarned`/`elapsedTimeSeconds`/`hasWaveProgress`), the chrono-aware entity-tick loop (via a pure-domain `EntityProtocol` seam), the collision sweep, the UW lifecycle timers (cooldown + effect-duration countdown + auto-trigger readiness), and the `SimulationEvent` flow (replacing `GameEngine`'s two `@Volatile` callbacks `onStepReward`/`onBossKilled` with `Simulation.events: SharedFlow<SimulationEvent>` collected by `BattleViewModel`) into the new pure-domain `domain/battle/engine/Simulation`; `GameEngine` delegates that surface and is now a thin presentation/render shell (`roundOver` + reward formulas + UW activation side-effects stay in the engine). Remaining substantial items: V1X-12 cloud save (schema v11→v12), V1X-13 i18n. **V1X-13 i18n phase 1 COMPLETE 2026-06-04:** notification strings (PR #109) + the `domain/Strings` engine-internal floating-text seam (PR #110, ADR-0014) + the battle + workshop Compose surfaces migrated to `stringResource` (PR #111, ~45 resources) + the `HardcodedText` lint-as-error guard (this PR — `lint { error += "HardcodedText" }`; empirically XML-only, does NOT catch Compose `Text()`, documented in `docs/architecture.md`). Phases 2/3 (remaining screens, then first non-English locale) deferred to v1.3/v1.4.)*
- [ ] **Plan 32: CI/CD Pipeline (GitHub Actions)** *(authored 2026-06-03 — `docs/plans/plan-32-ci.md` + ADR-0018. Three lanes: `ci.yml` PR gate (lint + unit + assembleDebug + schema-drift), `instrumented.yml` emulator suite (blocking-on-PR + nightly), `release.yml` signed bundleRelease → Play internal track on a `v*` tag. SHA-pinned actions + Dependabot. Merged to `main` via PR #100 (both CI checks green on GitHub runners; local gate also 867 tests — surfaced + fixed a latent `lintDebug` error). Repo secrets + `release` environment + branch protection + Play service account completed 2026-06-04 (9 secrets, `build-and-test` + `connected` required on `main`); `versionCode` bumped 16 → 17 — pipeline fully live (a `v*` tag now ships a signed AAB to the Play internal track).)*
