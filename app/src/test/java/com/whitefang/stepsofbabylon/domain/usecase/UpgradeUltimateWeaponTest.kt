...
class UpgradeUltimateWeaponTest {
    ...
    @Test
    fun `upgrade succeeds with sufficient stones`() =
        runTest {
    ...
    @Test
    fun `upgrade fails at max path level`() =
        runTest {
            assertFalse(sut(UltimateWeaponType.DEATH_WAVE, UWPath.DAMAGE, 10, 99999))
        }

    @Test
    fun `upgrade fails with insufficient stones`() =
        runTest {
    ...
    @Test
    fun `upgrade cost scales with path level`() =
        runTest {
    ...
    @Test
    fun `each path upgrades independently`() =
        runTest {
            assertTrue(sut(UltimateWeaponType.DEATH_WAVE, UWPath.DAMAGE, 1, 10000))
