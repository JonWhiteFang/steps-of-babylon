...
class OpenCardPackTest {
    ...
    @Test
    fun `new cards are added to repository`() =
        runTest {
            val result = useCase(PackTier.COMMON, 1000)
            assertTrue(result is OpenCardPack.Result.Opened)
    ...
    @Test
    fun `duplicate cards increment copy count`() =
        runTest {
    ...
    @Test
    fun `free pack skips gem deduction`() =
        runTest {
    ...
    @Test
    fun `R236 success routes through the atomic open-pack path`() =
        runTest {
            val result = useCase(PackTier.COMMON, 1000)
            assertTrue(result is OpenCardPack.Result.Opened)
    ...
    @Test
    fun `R122 stale snapshot does not grant a free pack`() =
        runTest {
    ...
    @Test
    fun `issue 18 regression - owned cards are persisted even when queried fresh from DB`() =
        runTest {
