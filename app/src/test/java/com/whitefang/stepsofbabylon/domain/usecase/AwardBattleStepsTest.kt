...
class AwardBattleStepsTest {
    ...
    @Test
    fun `first call credits the full amount`() =
        runTest {
    ...
    @Test
    fun `credits 0 and does not touch wallet once cap is hit`() =
        runTest {
            val (useCase, player, repo) = sut()
    ...
    @Test
    fun `partial credit when remaining is smaller than amount`() =
        runTest {
            val (useCase, player, repo) = sut()
    ...
    @Test
    fun `date rollover resets the per-day counter`() =
        runTest {
            val (useCase, player, repo) = sut()
            // Exhaust today's cap.
            repo.seedBattleStepsEarned(today, AwardBattleSteps.DAILY_BATTLE_STEP_CAP)
    ...
    @Test
    fun `zero or negative amounts are no-ops`() =
        runTest {
            val (useCase, player, repo) = sut()
    ...
    @Test
    fun `repo is incremented by credited amount not requested amount`() =
        runTest {
    ...
    @Test
    fun `FakeTimeProvider drives the cap reset when caller omits today`() =
        runTest {
    ...
    @Test
    fun `successful credit goes through atomic path and bypasses the legacy split path`() =
        runTest {
            val (useCase, player, repo) = sut()
    ...
    @Test
    fun `two concurrent kills on exactly one headroom - only one credits`() =
        runTest {
            val playerRepo = FakePlayerRepository(PlayerProfile(stepBalance = 0L))
            val repo = FakeStepRepository(linkedPlayer = playerRepo)
