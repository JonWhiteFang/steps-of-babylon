...
class UpdateCompleteResearchMissionProgressTest {
    ...
    @Test
    fun `R303 does NOT tick when completedCount is 0`() =
        runTest {
    ...
    @Test
    fun `R303 does NOT tick when completedCount is negative`() =
        runTest {
    ...
    @Test
    fun `R303 ticks to 1 when completedCount is 1`() =
        runTest {
            useCase(completedCount = 1, today = today)
    ...
    @Test
    fun `R303 caps progress at target when multiple research complete in one batch`() =
        runTest {
    ...
    @Test
    fun `R303 is a no-op when no mission row exists for the given date`() =
        runTest {
