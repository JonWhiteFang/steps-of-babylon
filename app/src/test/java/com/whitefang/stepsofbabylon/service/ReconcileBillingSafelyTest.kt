...
class ReconcileBillingSafelyTest {
    @Test
    fun `triggers reconcilePendingPurchases exactly once`() =
        runTest {
    ...
    @Test
    fun `a thrown reconcile does not propagate (best-effort, never crashes the worker)`() =
        runTest {
            val throwing =
                object : BillingManager {
                    ...
                    override suspend fun reconcilePendingPurchases(): Unit = throw RuntimeException("boom")
                }
    ...
    @Test
    fun `a hanging reconcile is time-bounded, not left to hang`() =
        runTest {
