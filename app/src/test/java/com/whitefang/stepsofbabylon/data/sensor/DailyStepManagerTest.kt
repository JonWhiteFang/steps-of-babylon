...
class DailyStepManagerTest {
    ...
    @Test
    fun `widget receives real step balance after crediting`() =
        runTest {
    ...
    @Test
    fun `widget balance accumulates across multiple credits`() =
        runTest {
            manager.recordSteps(100, baseTime)
    ...
    @Test
    fun `walking mission progress updates on step credit`() =
        runTest {
    ...
    @Test
    fun `walking mission completes when target reached`() =
        runTest {
            val today = manager.todayDate()
            dailyMissionDao.insert(
                DailyMissionEntity(
    ...
    @Test
    fun `battle mission is not updated by step credits`() =
        runTest {
            val today = manager.todayDate()
            dailyMissionDao.insert(
                DailyMissionEntity(
                    date = today,
                    missionType = DailyMissionType.REACH_WAVE_30.name,
                    target = 30,
                    rewardGems = 3,
                ),
            )
    ...
    @Test
    fun `activity minutes credit correct step-equivalents`() =
        runTest {
    ...
    @Test
    fun `duplicate activity-minute call produces zero additional credits`() =
        runTest {
    ...
    @Test
    fun `incremental activity-minute call credits only delta`() =
        runTest {
            manager.recordActivityMinutes(mapOf("cycling" to 10), 500)
    ...
    @Test
    fun `combined sensor and activity-minute credits respect 50k ceiling`() =
        runTest {
    ...
    @Test
    fun `process restart does not re-credit activity minutes`() =
        runTest {
            val minutes = mapOf("cycling" to 10)
            manager.recordActivityMinutes(minutes, 500)
            assertEquals(500L, playerRepo.getStepBalance())
    ...
    @Test
    fun `activity-minute credits trigger walking mission progress`() =
        runTest {
            val today = manager.todayDate()
            dailyMissionDao.insert(
                DailyMissionEntity(
    ...
    @Test
    fun `activity-minute credits trigger widget updates`() =
        runTest {
            manager.recordActivityMinutes(mapOf("cycling" to 10), 500)
    ...
    @Test
    fun `already completed mission is not re-updated`() =
        runTest {
            val today = manager.todayDate()
            dailyMissionDao.insert(
                DailyMissionEntity(
    ...
    @Test
    fun `background pipeline grants Season Pass bonus Gems when walking 1000+ steps`() =
        runTest {
            ...
            assertEquals(
                11L,
                gemsAfter - gemsBefore,
                "Season Pass owner should receive streak (1) + daily bonus (10) = 11 Gems",
            )
        }

    @Test
    fun `background pipeline grants baseline streak Gems without Season Pass`() =
        runTest {
    ...
    @Test
    fun `expired Season Pass does not grant daily bonus`() =
        runTest {
    ...
    @Test
    fun `R211 rollback baseline suppresses the gem award at the manager level`() =
        runTest {
    ...
    @Test
    fun `RO08 STEP_MULTIPLIER level 0 leaves credited steps unchanged`() =
        runTest {
    ...
    @Test
    fun `V1X18 STEP_MULTIPLIER level 50 grants asymptotic ~92 percent walking bonus`() =
        runTest {
    ...
    @Test
    fun `V1X18 STEP_MULTIPLIER level 200 asymptotes near +100 percent`() =
        runTest {
    ...
    @Test
    fun `RO08 STEP_MULTIPLIER bonus is capped by the 50k daily ceiling`() =
        runTest {
    ...
    @Test
    fun `RO08 STEP_MULTIPLIER does NOT apply to activity minutes`() =
        runTest {
    ...
    @Test
    fun `R251 trusted credit of a large gap is NOT clamped to the per-minute cap`() =
        runTest {
    ...
    @Test
    fun `R251 trusted credit still respects the 50k daily ceiling`() =
        runTest {
    ...
    @Test
    fun `R251 trusted credit applies the STEP_MULTIPLIER bonus`() =
        runTest {
            workshopRepo.upgrades.value =
                mapOf(com.whitefang.stepsofbabylon.domain.model.UpgradeType.STEP_MULTIPLIER to 50)
    ...
    @Test
    fun `R251 trusted credit does NOT record an anti-cheat rate rejection`() =
        runTest {
            manager.recordTrustedSteps(4_000, baseTime)
    ...
    @Test
    fun `R251 trusted credit of a non-positive delta is a no-op`() =
        runTest {
    ...
    @Test
    fun `RO11 STEP_EFFICIENCY level 5 grants a 10 percent walking bonus on its own`() =
        runTest {
    ...
    @Test
    fun `V1X18 STEP_MULTIPLIER and STEP_EFFICIENCY stack additively under the cap`() =
        runTest {
    ...
    @Test
    fun `V1X18 combined STEP_MULTIPLIER plus STEP_EFFICIENCY caps at +100 percent`() =
        runTest {
