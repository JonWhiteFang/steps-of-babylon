...
class WorkshopRepositoryImplTest {
    ...
    @Test
    fun `issue 20 - ensureUpgradesExist seeds every UpgradeType on a completely empty table (fresh install path)`() =
        runTest {
    ...
    @Test
    fun `issue 20 - ensureUpgradesExist inserts the new RAPID_FIRE enum on an upgrade-from-v8 install`() =
        runTest {
            val dao = FakeWorkshopDao()
            val repo = WorkshopRepositoryImpl(dao, playerProfileDao)
    ...
    @Test
    fun `issue 20 - ensureUpgradesExist is idempotent when every UpgradeType is already seeded`() =
        runTest {
            val dao = FakeWorkshopDao()
            val repo = WorkshopRepositoryImpl(dao, playerProfileDao)
    ...
    @Test
    fun `issue 20 - ensureUpgradesExist preserves an unknown upgradeType row in the table`() =
        runTest {
            ...
            val seededCount =
                rows.count {
                    it.upgradeType != "RETIRED_ENUM_NAME" &&
                        it.upgradeType != UpgradeType.DAMAGE.name
                }
