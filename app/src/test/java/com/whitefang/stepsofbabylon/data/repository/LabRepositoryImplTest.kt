...
class LabRepositoryImplTest {
    ...
    @Test
    fun `issue 20 sibling - ensureResearchExists seeds every ResearchType on a completely empty table`() = runTest {
        val dao = FakeLabDao()
        ...
        assertEquals(
            ResearchType.entries.size, rows.size,
    ...
    @Test
    fun `issue 20 sibling - ensureResearchExists inserts new MULTISHOT_RESEARCH and BOUNCE_RESEARCH on upgrade-from-v8 install`() = runTest {
        val dao = FakeLabDao()
        val repo = LabRepositoryImpl(dao)
        ...
        val historicalRows = historicalEnums.mapIndexed { i, type ->
            ...
            if (i == 0) {
                LabResearchEntity(
                    researchType = type.name, level = 3,
                    startedAt = 1_000_000L, completesAt = 1_001_000L,
                )
            } else {
        ...
        historicalRows.forEach { historical ->
            ...
            assertEquals(historical.completesAt, current.completesAt, "active-research completesAt must be preserved")
        }
    }

    @Test
    fun `issue 20 sibling - ensureResearchExists is idempotent when every ResearchType is already seeded`() = runTest {
        val dao = FakeLabDao()
        val repo = LabRepositoryImpl(dao)
