...
class DailyMissionDaoTest {
    ...
    @Test
    fun `inserting a duplicate date and missionType keeps only one row`() = runTest {
        val date = "2026-06-10"
    ...
    @Test
    fun `distinct missionTypes for the same date all persist`() = runTest {
        val date = "2026-06-10"
        missionDao.insert(DailyMissionEntity(date = date, missionType = "WALK_5000", target = 5000))
    ...
    @Test
    fun `two raced generation batches for the same date yield three missions not six`() = runTest {
        val date = "2026-06-10"
    ...
    @Test
    fun `generateForDate is idempotent — a second call adds no duplicate rows`() = runTest {
        val date = "2026-06-10"
