...
class DailyStepDaoTest {
     ...
     */
    @Test
    fun `incrementBattleSteps succeeds on empty table`() =
        runTest {
     ...
     */
    @Test
    fun `creditBattleStepsAtomic credits successfully on empty table`() =
        runTest {
            playerDao.upsert(PlayerProfileEntity(id = 1))

            val credited =
                dao.creditBattleStepsAtomic(
                    date = today,
                    requested = 5L,
                    dailyCap = 2_000L,
                    playerDao = playerDao,
     ...
     */
    @Test
    fun `creditBattleStepsAtomic preserves existing sensor data`() =
        runTest {
            playerDao.upsert(PlayerProfileEntity(id = 1))
     ...
     */
    @Test
    fun `creditBattleStepsAtomic returns partial credit near the cap`() =
        runTest {
            playerDao.upsert(PlayerProfileEntity(id = 1))
    ...
    @Test
    fun `creditBattleStepsAtomic returns zero when cap already exhausted`() =
        runTest {
            playerDao.upsert(PlayerProfileEntity(id = 1))
    ...
    @Test
    fun `setSensorAndCreditedSteps creates the row on an empty table`() =
        runTest {
    ...
    @Test
    fun `setHealthConnectSteps creates the row on an empty table`() =
        runTest {
    ...
    @Test
    fun `setActivityMinutes creates the row on an empty table`() =
        runTest {
    ...
    @Test
    fun `setEscrow creates the row on an empty table`() =
        runTest {
    ...
    @Test
    fun `setHealthConnectSteps preserves sensor and credited columns`() =
        runTest {
     ...
     */
    @Test
    fun `disjoint-column writers from the same stale snapshot do not clobber each other`() =
        runTest {
    ...
    @Test
    fun `column-targeted writers preserve the battle and boss counters`() =
        runTest {
    ...
    @Test
    fun `repeated setSensorAndCreditedSteps overwrites (not increments) its own columns`() =
        runTest {
