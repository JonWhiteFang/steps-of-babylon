...
class LabRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun getResearchLevel(type: ResearchType): Int = dao.getByType(type.name).first()?.level ?: 0

        override suspend fun getActiveResearchCount(): Int = dao.getActive().first().size
