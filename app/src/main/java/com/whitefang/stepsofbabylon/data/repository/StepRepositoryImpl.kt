...
class StepRepositoryImpl
    ...
    constructor(
        ...
        override fun observeHistory(
            startDate: String,
            endDate: String,
        ): Flow<List<DailyStepSummary>> = dao.getRange(startDate, endDate).map { list -> list.map { it.toDomain() } }
        ...
        override suspend fun getDailyRecord(date: String): DailyStepSummary? = dao.getByDateOnce(date)?.toDomain()
        ...
        override suspend fun sumCreditedSteps(
            startDate: String,
            endDate: String,
        ): Long = dao.sumCreditedSteps(startDate, endDate)

        override suspend fun creditBattleStepsAtomic(
            date: String,
            requested: Long,
            dailyCap: Long,
        ): Long = dao.creditBattleStepsAtomic(date, requested, dailyCap, playerDao = playerProfileDao)

        override suspend fun creditBossPowerStonesAtomic(
            date: String,
            requested: Long,
            dailyCap: Long,
        ): Long = dao.creditBossPowerStonesAtomic(date, requested, dailyCap, playerDao = playerProfileDao)

        private fun DailyStepRecordEntity.toDomain() =
            DailyStepSummary(
                date = date,
