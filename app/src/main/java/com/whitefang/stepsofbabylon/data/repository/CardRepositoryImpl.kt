...
class CardRepositoryImpl
    ...
    constructor(
        ...
        override fun observeAllCards(): Flow<List<OwnedCard>> = dao.getAll().map { list -> list.map { it.toDomain() } }

        override fun observeEquippedCards(): Flow<List<OwnedCard>> =
            dao.getEquipped().map { list -> list.map { it.toDomain() } }

        override suspend fun hasCard(type: CardType): Boolean = dao.getByType(type.name) != null

        override suspend fun addCard(type: CardType): Long = dao.insert(CardInventoryEntity(cardType = type.name))

        override suspend fun openCardPackAtomic(
            gemCost: Long,
            cardTypeNames: List<String>,
        ): List<Boolean>? = dao.openCardPackAtomic(gemCost, cardTypeNames, playerDao)
        ...
        override suspend fun decrementCopiesAndLevelUp(
            id: Int,
            amount: Int,
        ): Boolean = dao.decrementCopiesAndLevelUp(id, amount) > 0
        ...
        private fun CardInventoryEntity.toDomain() =
            OwnedCard(
                id = id,
