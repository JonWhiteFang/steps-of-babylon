...
class DailyStepManager
    ...
    constructor(
        ...
        private val trackWeeklyChallenge by lazy {
            TrackWeeklyChallenge(weeklyChallengeRepository, stepRepository, playerRepository)
        }
         ...
         */
        suspend fun recordTrustedSteps(
            rawDelta: Long,
            timestampMs: Long,
        ) {
            if (rawDelta <= 0) return
            ...
            try {
                ...
                dropState =
                    dropState.copy(
                        ...
                        milestoneTriggered =
                            dropState.milestoneTriggered || (drop?.trigger == SupplyDropTrigger.DAILY_MILESTONE),
                    )
            } catch (e: Exception) {
                onPipelineError("supply-drop", e)
            }
            ...
            try {
                ...
                val timeVerdict =
                    TimeIntegrity.evaluate(
                        antiCheatPrefs.readTimeBaseline(),
                        antiCheatPrefs.currentTimeReading(),
                    )
                ...
                onPipelineError("economy", e)
            }

            // Walking mission progress
            try {
                updateWalkingMissions()
            } catch (e: Exception) {
                onPipelineError("mission", e)
            }
        }
        ...
        private suspend fun applyStepMultiplier(baseCredit: Long): Long {
            ...
            val workshopBonus =
                com.whitefang.stepsofbabylon.domain.battle.engine.SimulationMath.stepMultiplierBonus(
                    wsLevel,
                )
