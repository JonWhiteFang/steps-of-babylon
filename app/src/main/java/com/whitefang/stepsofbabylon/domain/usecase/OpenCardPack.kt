...
enum class PackTier(
    val gemCost: Long,
    val commonWeight: Double,
    val rareWeight: Double,
    val epicWeight: Double,
) {
...
class OpenCardPack(
    ...
    suspend operator fun invoke(
        ...
        val isNewFlags =
            cardRepository.openCardPackAtomic(gemCost, rolledTypes.map { it.name })
    ...
    private fun packTierToRarity(packTier: PackTier): CardRarity =
        when (packTier) {
